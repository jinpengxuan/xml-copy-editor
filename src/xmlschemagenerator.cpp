...
const wxString &XmlSchemaGenerator::generate(Grammar::GrammarType grammarType,
  ...
  if (!wxIsEmpty(encoding))
    source.setEncoding((const XMLCh *)WrapXerces::toString(encoding).GetData());
  try {
  ...
  size_t size = len / 3;
  if (size > maxReservedSchemaBuffer)
    size = maxReservedSchemaBuffer;
  mSchema.Alloc(size);
  ...
  if (mGrammarType == Grammar::SchemaGrammarType)
    mSchema << _T("<?xml version=\"1.0\" encoding=\"UTF-8\"?>") << getEOL()
            << _T("<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" ")
               _T("elementFormDefault=\"qualified\">")
            << getEOL();
...
void XmlSchemaGenerator::generateData(const DOMElement &element,
  ...
  if (mElements[name].name.empty()) { // Only generate data once
...
void XmlSchemaGenerator::generateData(const wxString &elementName,
  ...
  for (; elmtItr != data.nodes.end(); ++elmtItr) {
    ...
    for (; child != NULL; child = child->getNextSibling()) {
      ...
      if (type != DOMNode::ELEMENT_NODE) {
        if (type == DOMNode::TEXT_NODE) { // Check for mixed content
  ...
  for (; elmtItr != data.nodes.end(); ++elmtItr) {
    ...
    for (attrItr = attrMap.begin(); attrItr != attrMap.end(); ++attrItr) {
      if (attrs->getNamedItem(
              (const XMLCh *)WrapXerces::toString(attrItr->first).GetData()) ==
          NULL) {
        optAttrs.insert(attrItr->first);
      }
    }
  ...
  if (mGrammarType == Grammar::DTDGrammarType) {
    ...
    mSchema << data.schema;
  } else if (!mInlineSimpleType) { // Or wait until all data are available
...
void XmlSchemaGenerator::generateSchema(ElmtData &data, size_t nIndent) {
  ...
  if (!data.children.empty()) {
    ...
    // if (minOccurs != 1) schema << _T(" minOccurs=\"") << minOccurs <<
    // _T("\""); if (maxOccurs > 1) schema << _T(" maxOccurs=\"unbounded\"");
    if (minTotal == 0)
    ...
    for (; seqItr != data.sequence.end(); ++seqItr) {
      ...
      addIndent(schema, nIndent);
      if (mInlineSimpleType) { // Check if it's a simple type
        const ElmtData *childElmt = &mElements[*seqItr];
        if (childElmt->children.empty() && childElmt->attrMap.empty()) {
  ...
  for (; attrItr != data.attrMap.end(); ++attrItr) {
    ...
    if (attrItr->second != NULL) {
      schema << _T(" default=\"") << WrapXerces::toString(attrItr->second)
             << _T("\"");
...
void XmlSchemaGenerator::generateDTD(ElmtData &data, size_t WXUNUSED(nIndent)) {
  ...
  if (!data.attrMap.empty()) {
    const static wxString indent = wxString(getEOL()) + _T("          ");
    ...
    for (; attrItr != data.attrMap.end(); ++attrItr) {
      ...
      if (attrItr->second != NULL) // Has default value
        schema << _T(" \"") << WrapXerces::toString(attrItr->second)
               << _T("\"");
      else if (data.optAttrs.find(attrItr->first) == data.optAttrs.end())
        schema << _T(" #REQUIRED");
      else
...
bool XmlSchemaGenerator::getSequence(
  ...
  do {
    ...
    for (itr = elmtMap.begin(); itr != elmtMap.end(); ++itr) {
      seqFindItr = std::find(sequence.begin(), sequence.end(), itr->first);
      ...
      for (; prevItr != prevEnd;
           ++prevItr) { // Find last index of dependent elements
        seqFindItr = std::find(sequence.begin(), sequence.end(), *prevItr);
        if (seqFindItr != sequence.end()) {
        ...
        if (previous.find(itr->first) == previous.end()) { // Not a deadlock
          retry = true;
          break;
        } else {
          deadlock = true;
