...
XmlCopyImg::XmlCopyImg(const wxString &blankImage, const wxString &imagesDir,
                       const wxString &mediaDir, const wxString &path,
                       bool parseDeclaration, bool expandInternalEntities,
                       size_t size)
    : d(new ImgData()) {
  wxCopyFile(blankImage,
...
void XMLCALL XmlCopyImg::xmldeclhandler(void *data, const XML_Char *version,
...
void XMLCALL XmlCopyImg::defaulthandler(void *data, const XML_Char *s,
                                        int len) {
  ImgData *d;
  d = (ImgData *)data;
...
void XMLCALL XmlCopyImg::start(void *data, const XML_Char *el,
                               const XML_Char **attr) {
  ImgData *pd;
  pd = (ImgData *)data;
  ...
  for (; *attr; attr += 2) {
    ...
    if (!strcmp(el, "img") && !strcmp(*attr, "src")) {
      ...
      if (!wxFileExists(wideFile)) {
        ...
        wideDestination =
            pd->imagesDir + wxFileName::GetPathSeparator() + fn.GetFullName();
        wxCopyFile(wideFile, wideDestination, true);
      }

      if (wideDestination.Contains(_T(".eps" )) ||
        ...
        if (wxExecute(cmd, wxEXEC_SYNC)) {
          wxString error;
          error.Printf(
              _T("Please ensure that ImageMagick is installed on your system ")
              _T("and that the conversion tool is located at:\n\n%s\n\nYou ")
              _T("can download ImageMagick from:\n\n%s\n\nIf your document ")
              _T("contains encapsulated PostScript (EPS) graphics, you need ")
              _T("to install GhostScript as well.\n\nYou can download ")
              _T("GhostScript from:\n\n%s"),
              IMAGEMAGICK_CONVERT_PATH, IMAGEMAGICK_INSTALL_URL,
    ...
    else if (!strcmp(el, "a") && !strcmp(*attr, "href") &&
      ...
      if (!wxFileExists(wideFile)) {
        ...
        value += fn.GetFullName().mb_str(wxConvUTF8);
        wideDestination =
            pd->mediaDir + wxFileName::GetPathSeparator() + fn.GetFullName();
        wxCopyFile(wideFile, wideDestination, true);
      }
    } // a + href
