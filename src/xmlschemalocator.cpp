...
XmlSchemaLocator::XmlSchemaLocator(const char *encoding)
    : WrapExpat(encoding, true), d(new SchemaLocatorData()) {
  d->parser = p;
...
void XMLCALL XmlSchemaLocator::starthandler(void *data, const XML_Char *el,
                                            const XML_Char **attr) {
  SchemaLocatorData *d;
  d = (SchemaLocatorData *)data;

  while (*attr) {
    if (!strcmp(*attr, "http://www.w3.org/2001/"
                       "XMLSchema-instance:noNamespaceSchemaLocation") ||
        !strcmp(*attr,
...
std::string XmlSchemaLocator::getSchemaLocation() { return d->schemaLocation; }
