...
MyNotebook::MyNotebook(wxWindow *parent, wxWindowID id, const wxPoint &position,
                       const wxSize &size, int style)
    : wxAuiNotebook(parent, id, position, size, style) {
  rightClickPage = -1;

  wxTheApp->Connect(wxEVT_KEY_DOWN, wxKeyEventHandler(MyNotebook::OnKeyDown),
                    NULL, this);
}
