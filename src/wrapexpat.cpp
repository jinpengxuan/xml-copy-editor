...
WrapExpat::WrapExpat(const char *encoding, bool nameSpaceAware) {
  p = (nameSpaceAware) ? XML_ParserCreateNS(encoding, (XML_Char)':')
...
bool WrapExpat::parse(const string &buffer, bool isFinal) {
  ...
  /*
    ...
    if (XML_Parse(p, buffer.c_str(), buffer.size(), isFinal) ==
    XML_STATUS_ERROR) return false; return true;
  */
}

pair< int, int > WrapExpat::getErrorPosition() {
  return make_pair(XML_GetCurrentLineNumber(p), // - 1,
...
bool WrapExpat::isEncodingError() {
  ...
  return (errorCode == XML_ERROR_UNKNOWN_ENCODING ||
