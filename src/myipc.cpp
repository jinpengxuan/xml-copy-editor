...
MyServerConnection::MyServerConnection() : wxConnection(), mFrameWnd(0) {
...
bool MyServerConnection::OnPoke(const wxString &WXUNUSED(topic),
                                const wxString &item, IPCData *data,
                                IPCSize_t size, wxIPCFormat WXUNUSED(format)) {
  if (!wxTheApp)
    return false;
  MyFrame *frame;
  ...
  if (item == (wxString)IPC_NO_FILE) {
    ...
    // frame->addToFileQueue ( ( wxString& ) item ); // prevent event loop
    // problems
  }
#ifndef __WXMSW__
#ifdef HAVE_GTK2
...
IPCData *
MyServerConnection::OnRequest(const wxString &WXUNUSED(topic),
                              const wxString &item, IPCSize_t *size,
...
bool MyClientConnection::OnAdvise(const wxString &WXUNUSED(topic),
                                  const wxString &WXUNUSED(item),
...
bool MyClient::talkToServer(int argc, const wxChar *const *argv) {
  MyClientConnection *connection = (MyClientConnection *)MakeConnection(
      _T ( "localhost" ), IPC_SERVICE, IPC_TOPIC);
  ...
  if (size == sizeof(GdkNativeWindow)) {
    ...
    if (window) {
      gdk_x11_window_set_user_time(window, XTimeNow());
      // gdk_x11_get_server_time ( window ) );
      gdk_window_show(window);
      gdk_window_raise(window);
