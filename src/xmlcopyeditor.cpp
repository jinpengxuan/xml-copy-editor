...
typedef size_t universal_iconv(iconv_t cd, char **inbuf, size_t *inbytesleft,
...
EVT_MENU_RANGE(ID_VALIDATE_PRESET1, ID_VALIDATE_PRESET9,
               MyFrame::OnValidatePreset)
EVT_MENU_RANGE(ID_COLOR_SCHEME_DEFAULT, ID_COLOR_SCHEME_NONE,
               MyFrame::OnColorScheme)

EVT_MENU_RANGE(ID_SPLIT_TAB_TOP, ID_SPLIT_TAB_LEFT, MyFrame::OnSplitTab)

EVT_UPDATE_UI_RANGE(ID_REPLACE, ID_GLOBAL_REPLACE,
                    MyFrame::OnUpdateReplaceRange)
...
EVT_UPDATE_UI_RANGE(ID_SPLIT_TAB_TOP, ID_SPLIT_TAB_LEFT,
                    MyFrame::OnUpdateCloseAll)
EVT_UPDATE_UI(wxID_REVERT, MyFrame::OnUpdateUndo)
EVT_UPDATE_UI(wxID_SAVE,
              MyFrame::OnUpdateDocRange) // always allow save if doc present
...
MyApp::MyApp()
    : checker(NULL), server(NULL), singleInstanceCheck(false), lang(0)
#if defined(__WXMSW__) && !wxCHECK_VERSION(2, 9, 0)
      ,
      config(new wxFileConfig(
          _T ( ".xmlcopyeditor" ))) //( _T ( "SourceForge Project\\XML Copy
                                    //Editor" ) ) )
#else
      ,
...
bool MyApp::OnInit() {
  ...
  if (config.get()) {
    singleInstanceCheck =
        config->Read(_T ( "singleInstanceCheck" ), singleInstanceCheck);
  ...
  try {
    ...
    frame = new MyFrame(_("XML Copy Editor"), config.get(), myLocale,
                        singleInstanceCheck, lang);
    frame->Show(true);
    ...
    errorString = _("XML Copy Editor has encountered the following error and "
                    "needs to close: ");
    errorString += wideWhat;
    errorString += _T ( "." );
#ifdef __WXMSW__
    ::MessageBox(NULL, errorString, _("Error"), MB_ICONERROR | MB_TASKMODAL);
#else
...
#ifdef __WXMSW__
  ::MessageBox(
      NULL, _("XML Copy Editor has encountered an error and needs to close."),
      _("Error"), MB_ICONERROR | MB_TASKMODAL);
#else
  wxMessageBox(
...
bool MyApp::OnExceptionInMainLoop() {
  try {
    ...
    ::MessageBox(NULL,
                 _("The operating system has turned down a request for "
                   "additional memory"),
                 _("Out of memory"), MB_ICONERROR);
    return true;
  }
#endif
  catch (exception &e) {
    ...
    errorString += wideWhat;
    errorString += _(".\n\nSelect \"Abort\" to exit, \"Retry\" to close this "
                     "window and \"Ignore\" to continue.");
#ifdef __WXMSW__
    int ret = ::MessageBox(NULL, errorString, _("Error"),
    ...
    wxString otherError(
        _("XML Copy Editor has encountered an error and needs to close."));
#ifdef __WXMSW__
    ::MessageBox(NULL, otherError, _("Error"), MB_ICONERROR);
    return false;
#else
    wxMessageBox(otherError, _("Error"), wxICON_ERROR);
    return false;
#endif
  }
  return false;
...
#ifndef __WXMSW__
void MyApp::HandleEvent(wxEvtHandler *handler, wxEventFunction func,
                        wxEvent &event) const {
  try {
    wxApp::HandleEvent(handler, func, event);
  } catch (std::bad_alloc &) {
    wxMessageBox(_("The operating system has turned down a request for "
                   "additional memory"),
                 _("Out of memory"), wxICON_ERROR);
    return;
  } catch (std::exception &e) {
    std::string s(e.what());
    wxString ws = wxString(s.c_str(), wxConvUTF8, s.size());
    wxMessageBox(ws, _("Error"), wxICON_ERROR);
    return;
  } catch (...) {
    throw;
  }
  ...
  public:
    ...
    bool HasMsgCatalogInDir(const wxString &dir, const wxString &catelog) {
      return wxFileName(dir, catelog, _T ( "mo" )).FileExists() ||
             wxFileName(dir + wxFILE_SEP_PATH + _T ( "LC_MESSAGES" ), catelog,
                        _T ( "mo" ))
                 .FileExists();
    }
    const std::set< const wxLanguageInfo * > &operator()() {
      return translations;
    }
...
MyFrame::MyFrame(const wxString &title, wxFileConfig *configParameter,
                 wxLocale &locale, bool singleInstanceCheckParameter,
                 int langParameter)
    : wxFrame(NULL, wxID_ANY, title), config(configParameter), myLocale(locale),
      singleInstanceCheck(singleInstanceCheckParameter), lang(langParameter),
      htmlPrinting(new wxHtmlEasyPrinting(wxEmptyString, this)), findDialog(0),
      helpController(new wxHtmlHelpController(
#ifdef __WXOSX__
          wxHF_CONTENTS | wxHF_INDEX | wxHF_SEARCH | wxHF_BOOKMARKS | wxHF_PRINT
#endif
          )),
      menuBar(0), toolBar(0), xmlMenu(0), mainBook(0),
  ...
  wxString defaultFont =
      wxSystemSettings::GetFont(wxSYS_ANSI_VAR_FONT).GetFaceName();
  ...
  {
    history.Load(*config);
    properties.insertCloseTag = config->Read(_T ( "insertCloseTag" ), true);
    properties.completion = config->Read(_T ( "completion" ), true);
    properties.number = config->Read(_T ( "number" ), true);
    properties.fold = config->Read(_T ( "fold" ), true);
    properties.foldCompact = config->Read(_T ( "foldCompact" ), true);
    properties.currentLine = config->Read(_T ( "currentLine" ), true);
    properties.highlightSyntax = config->Read(_T ( "highlightSyntax" ), true);
    properties.whitespaceVisible =
        config->Read(_T ( "whitespaceVisible" ), true);
    properties.indentLines = config->Read(_T ( "indentLines" ), true);
    ...
    properties.deleteWholeTag = config->Read(_T ( "deleteWholeTag" ), true);
    properties.validateAsYouType =
        config->Read(_T ( "validateAsYouType" ), true);
    properties.font = config->Read(_T ( "font" ), defaultFont);
    findRegex = config->Read(_T ( "findRegex" ), true);
    xpathExpression = config->Read(_T ( "xpathExpression" ), wxEmptyString);
    lastXslStylesheet = config->Read(_T ( "lastXslStylesheet" ), wxEmptyString);
    lastRelaxNGSchema = config->Read(_T ( "lastRelaxNGSchema" ), wxEmptyString);

    exportQuiet = config->Read(_T ( "exportQuiet" ), (long)true);
    exportMp3Album = config->Read(_T ( "exportMp3Album" ), (long)true);
    exportSuppressOptional =
        config->Read(_T ( "exportSuppressOptional" ), (long)true);
    exportHtml = config->Read(_T ( "exportHtml" ), (long)true);
    exportEpub = config->Read(_T ( "exportEpub" ), (long)true);
    exportRtf = config->Read(_T ( "exportRtf" ), (long)true);
    exportDoc = config->Read(_T ( "exportDoc" ), (long)true);
    exportFullDaisy = config->Read(_T ( "exportFullDaisy" ), (long)true);

    applicationDir = config->Read(_T ( "applicationDir" ),
    ...
    properties.wrap = config->Read(_T ( "wrap" ), longFalse);

    properties.zoom = config->Read(_T ( "zoom" ), valZoom);

    properties.colorScheme =
        config->Read(_T ( "colorScheme" ), COLOR_SCHEME_DEFAULT);

    globalReplaceAllDocuments =
        config->Read(_T ( "globalReplaceAllDocuments" ), longFalse);
    showFullPathOnFrame = config->Read(_T ( "showFullPathOnFrame" ), longFalse);
    findMatchCase = config->Read(_T ( "findMatchCase" ), longFalse);

    commandSync = config->Read(_T ( "commandSync" ), longFalse);
    commandOutput =
        config->Read(_T ( "commandOutput" ), ID_COMMAND_OUTPUT_IGNORE);
    ...
    ruleSetPreset = config->Read(_T ( "ruleSetPreset" ), _("Default style"));
    dictionaryPreset = config->Read(_T ( "dictionaryPreset" ), _("en_US"));

    filterPreset = config->Read(_T ( "filterPreset" ), _("(No filter)"));
    findData.SetFindString(config->Read(_T ( "findReplaceFind" ), _T ( "" )));
    findData.SetReplaceString(
        config->Read(_T ( "findReplaceReplace" ), _T ( "" )));

    toolbarVisible = config->Read(_T ( "toolbarVisible" ), true);
    ...
    notebookStyle =
        config->Read(_T ( "notebookStyle" ), ID_NOTEBOOK_STYLE_VC8_COLOR);
    ...
    expandInternalEntities =
        config->Read(_T ( "expandInternalEntities" ), true);
  ...
  {
    properties.insertCloseTag = properties.completion = properties.currentLine =
        properties.indentLines = properties.protectHiddenElements =
            properties.toggleLineBackground = properties.deleteWholeTag =
    ...
    findRegex = true;
    findMatchCase = globalReplaceAllDocuments = showFullPathOnFrame = false;
    toolbarVisible = true;
    protectTags = false;
  ...
  long style = wxAUI_NB_TOP | wxAUI_NB_TAB_SPLIT | wxAUI_NB_TAB_MOVE |
               wxAUI_NB_SCROLL_BUTTONS | wxAUI_NB_WINDOWLIST_BUTTON |
               wxAUI_NB_CLOSE_ON_ALL_TABS | wxNO_BORDER;

  mainBook = new MyNotebook(this, ID_NOTEBOOK, wxDefaultPosition, wxDefaultSize,
                            style);

  manager.AddPane(mainBook, wxAuiPaneInfo().CenterPane().PaneBorder(false).Name(
                                _T ( "documentPane" )));
  ...
  insertChildPanel =
      new InsertPanel(this, ID_INSERT_CHILD_PANEL, INSERT_PANEL_TYPE_CHILD);
  insertSiblingPanel =
      new InsertPanel(this, ID_INSERT_SIBLING_PANEL, INSERT_PANEL_TYPE_SIBLING);
  insertEntityPanel =
      new InsertPanel(this, ID_INSERT_ENTITY_PANEL, INSERT_PANEL_TYPE_ENTITY);
  ...
  manager.GetPane(locationPanel)
      .Name(_T ( "locationPane" ))
      .Show((restoreLayout) ? showLocationPane : true)
      .DestroyOnClose(false)
      .PinButton(true);
  manager.GetPane(locationPanel).dock_proportion = 1;

  manager.GetPane(insertChildPanel)
      .Name(_T ( "insertChildPane" ))
      .Show((restoreLayout) ? showInsertChildPane : true)
      .DestroyOnClose(false)
      .PinButton(true);
  manager.GetPane(insertChildPanel).dock_proportion = 1;

  manager.GetPane(insertSiblingPanel)
      .Name(_T ( "insertSiblingPane" ))
      .Show((restoreLayout) ? showInsertSiblingPane : true)
      .DestroyOnClose(false)
      .PinButton(true);
  manager.GetPane(insertSiblingPanel).dock_proportion = 1;

  manager.GetPane(insertEntityPanel)
      .Name(_T ( "insertEntityPane" ))
      .Show((restoreLayout) ? showInsertEntityPane : true)
      .DestroyOnClose(false)
      .PinButton(true);
  manager.GetPane(insertEntityPanel).dock_proportion = 1;

  // add (hidden) message pane
  htmlReport = new MyHtmlPane(this, ID_VALIDATION_PANE, wxDefaultPosition,
                              wxSize(-1, 48));
#ifndef __WXMSW__
  ...
  manager.AddPane(htmlReport, wxAuiPaneInfo()
                                  .Movable()
                                  .Bottom()
                                  .Hide()
                                  .Name(_T ( "messagePane" ))
...
#ifdef NEWFINDREPLACE
  findReplacePanel = new FindReplacePanel(this, ID_FIND_REPLACE_PANEL,
                                          &findData, true, findRegex);

  manager.AddPane((wxWindow *)findReplacePanel, wxAuiPaneInfo()
                                                    .Bottom()
                                                    .Hide()
  ...
  commandPanel = new CommandPanel(this, wxID_ANY,
                                  commandString, // tbd
                                  commandSync,   // tbd
                                  commandOutput  // tbd
  );
  manager.AddPane((wxWindow *)commandPanel, wxAuiPaneInfo()
                                                .Bottom()
                                                .Hide()
                                                .Caption(_T ( "Command" ))
                                                .DestroyOnClose(false)
...
MyFrame::~MyFrame() {
  ...
  config->Write(_T ( "protectHiddenElements" ),
                properties.protectHiddenElements);
  ...
  config->Write(_T ( "showLocationPane" ),
                manager.GetPane(locationPanel).IsShown());
  config->Write(_T ( "showInsertChildPane" ),
                manager.GetPane(insertChildPanel).IsShown());
  config->Write(_T ( "showInsertSiblingPane" ),
                manager.GetPane(insertSiblingPanel).IsShown());
  config->Write(_T ( "showInsertEntityPane" ),
                manager.GetPane(insertEntityPanel).IsShown());
    ...
    while ((c = *++s) != 0) {
      ...
      case '-':
        if (*++s == 'v') {
          ...
          std::cout
              << "Usage: xmlcopyeditor [--version --help -ws] [<file>] "
                 "[<file2>]"
              << std::endl
              << "Options -w (import Microsoft Word document) and -s (open "
                 "Spelling and style check) are provided for integration with "
                 "Microsoft Office and only available on Windows"
              << std::endl;
        }
        exit(0);
      default:
        messagePane(_("Unknown command line switch (expecting 'w', 's', "
                      "--version or --help)"),
                    CONST_STOP);
        return;
      }
...
void MyFrame::OnAbout(wxCommandEvent &WXUNUSED(event)) {
  ...
  info.AddDeveloper(
      _T ( "Gerald Schmidt (development) <gnschmidt at users dot sourceforge dot net>" ));
  info.AddDeveloper(
      _T ( "Matt Smigielski (testing) <alectrus at users dot sourceforge dot net>" ));
  info.AddDeveloper(
      _T ( "Justin Dearing (development) <j-pimp at users dot sourceforge dot net>" ));
  info.AddDeveloper(
      _T ( "Kev James (development) <kmjames at users dot sourceforge dot net>" ));
  info.AddDeveloper(
      _T ( "Anh Trinh (development) <ant271 at users dot sourceforge dot net>" ));
  info.AddDeveloper(
      _T ( "Zane U. Ji (development) <zaneuji at users dot sourceforge dot net>" ));
  info.AddTranslator(_T ( "Viliam Búr (Slovak) <viliam at bur dot sk>" ));
  info.AddTranslator(
      _T ( "David Håsäther (Swedish) <hasather at gmail dot com>" ));
  info.AddTranslator(
      _T ( "François Badier (French) <frabad at gmail dot com>" ));
  info.AddTranslator(
      _T ( "Thomas Wenzel (German) <thowen at users dot sourceforge.net>" ));
  info.AddTranslator(
      _T ( "SHiNE CsyFeK (Chinese Simplified) <csyfek at gmail dot com>" ));
  info.AddTranslator(
      _T ( "HSU PICHAN, YANG SHUFUN, CHENG PAULIAN, CHUANG KUO-PING, Marcus Bingenheimer (Chinese Traditional)" ));
  info.AddTranslator(
      _T ( "Serhij Dubyk (Ukrainian) <dubyk at library dot lviv dot ua>" ));
  info.AddTranslator(
      _T ( "Antonio Angelo (Italian) <aangelo at users dot sourceforge dot net>" ));
  info.AddTranslator(
      _T ( "Siarhei Kuchuk (Russian) <Cuchuk dot Sergey at gmail dot com>" ));
  info.AddTranslator(
      _T ( "Marcos Pérez González (Spanish) <marcos_pg at yahoo dot com>" ));
  info.AddTranslator(_T ( "Rob Elemans (Dutch) <relemans at gmail dot com>" ));
  info.AddTranslator(
      _T ( "Robert Falcó Miramontes <rfalco at acett dot org>" ));
  info.AddTranslator(
      _T ( "Khoem Sokhem (Khmer) <sokhem at open dot org dot kh>" ));
  info.AddTranslator(
      _T ( "Roger Sperberg (Khmer) <rsperberg at gmail dot com>" ));
...
void MyFrame::OnCheckWellformedness(wxCommandEvent &event) {
  ...
  if (!we.parse(utf8Buffer)) {
    ...
    --(posPair.first);
    int cursorPos = doc->PositionFromLine(posPair.first);
...
void MyFrame::OnPageClosing(
    wxAuiNotebookEvent
        &event) // wxNotebookEvent& event)//wxFlatNotebookEvent& event)
{
  ...
  if (doc->GetModify()) // CanUndo())
  {
    ...
    int answer = wxMessageBox(
        ...
        _("XML Copy Editor"), wxYES_NO | wxCANCEL | wxICON_QUESTION, this);
  ...
  */
  ...
  if (visibilityState == HIDE_ATTRIBUTES) {
    if (currentHiddenStatus != _("Attributes hidden"))
      status->SetStatusText(_("Attributes hidden"), STATUS_HIDDEN);
  } else if (visibilityState == HIDE_TAGS) {
    if (currentHiddenStatus != _("Tags hidden"))
      status->SetStatusText(_("Tags hidden"), STATUS_HIDDEN);
  } else {
  ...
  if (protectTags) {
    if (currentProtectedStatus != _("Tags locked"))
      status->SetStatusText(_("Tags locked"), STATUS_PROTECTED);
  } else {
  ...
  if (current != lastPos) {
    ...
    coordinates.Printf(_("Ln %i Col %i"), doc->LineFromPosition(current) + 1,
  ...
  if (!manager.GetPane(insertChildPanel).IsShown() &&
      !properties.validateAsYouType)
    return;
  ...
  if (!parent.empty() && properties.validateAsYouType &&
      doc->getValidationRequired()) {
  ...
  if (!parent.empty() && parentCloseAngleBracket != -1) {
    ...
    grandParentCloseAngleBracket = doc->getParentCloseAngleBracket(
...
void MyFrame::OnPasteNewDocument(wxCommandEvent &event) {
  ...
  if (!wxTheClipboard->IsSupported(wxDF_TEXT)) {
    messagePane(_("Cannot paste as new document: no text on clipboard"),
                CONST_STOP);
    return;
  }
  wxTextDataObject data;
...
void MyFrame::OnPrintPreview(wxCommandEvent &WXUNUSED(event)) {
  ...
  htmlPrinting->SetHeader(header, wxPAGE_ALL);
...
void MyFrame::OnFind(wxCommandEvent &WXUNUSED(event)) {
  ...
  findDialog = (std::auto_ptr< wxFindReplaceDialog >(
      new wxFindReplaceDialog(this, &findData, _("Find"))));
  findDialog->Show();
}
...
void MyFrame::OnImportMSWord(wxCommandEvent &event) {
  ...
  std::auto_ptr< wxFileDialog > fd(
      new wxFileDialog(this, _("Import Microsoft Word Document"), mLastDir,
                       _T ( "" ), _T ( "Microsoft Word (*.doc)|*.doc" ),
#if wxCHECK_VERSION(2, 9, 0)
...
void MyFrame::OnExport(wxCommandEvent &event) {
  ...
  wxString testDir =
      applicationDir + wxFileName::GetPathSeparator() + _T ( "daisy" );
  bool downloadLink = !wxDirExists(testDir);

  std::auto_ptr< ExportDialog > ed(new ExportDialog(
      this, exportStylesheet, exportFolder, exportQuiet, exportSuppressOptional,
      exportHtml, exportEpub, exportRtf, exportDoc, exportFullDaisy,
      exportMp3Album, downloadLink));
  ...
  if (!wd.run(tempFile, exportStylesheet, exportFolder, exportQuiet,
              exportSuppressOptional, exportEpub, exportRtf, exportDoc,
              exportFullDaisy, exportMp3Album)) {
    messagePane(_("[b]DAISY export stopped[/b]: ") + wd.getLastError(),
                CONST_STOP);
    return;
  }
  messagePane(_("DAISY export completed. Output files are stored in: [b]") +
                  exportFolder + _T ( "[/b]." ),
              CONST_INFO);
}
...
void MyFrame::importMSWord(const wxString &path) {
  ...
  wxString cmd = binDir + _T ( "doc2xml.exe \"" ) + completeSwapFileName +
                 _T ( "\" \"" ) + tempFileName.wideName() + _T ( "\"" );
  ...
  case 1:
    ...
    return;
  case 2:
    messagePane(_("A more recent version of Microsoft Word is required"),
                CONST_STOP);
    return;
  case 3:
    ...
    return;
  case 4:
    message.Printf(_("Microsoft Word cannot save [b]%s[/b] as XML"),
                   path.c_str());
    messagePane(message, CONST_STOP);
    return;
  case 5:
...
void MyFrame::OnExportMSWord(wxCommandEvent &event) {
  ...
  std::auto_ptr< wxFileDialog > fd(
      new wxFileDialog(this, _("Export Microsoft Word Document"), _T ( "" ),
                       _T ( "" ), _T ( "Microsoft Word (*.doc)|*.doc" ),
#if wxCHECK_VERSION(2, 9, 0)
  ...
  wxString cmd = binDir + _T ( "xml2doc.exe -v \"" ) + sourceFileName +
                 _T ( "\" \"" ) + path + _T ( "\"" );
...
void MyFrame::OnOptions(wxCommandEvent &WXUNUSED(event)) {
  ...
  std::auto_ptr< MyPropertySheet > mpsd(new MyPropertySheet(
      this, properties, applicationDir, rememberOpenTabs, libxmlNetAccess,
      singleInstanceCheck, saveBom, unlimitedUndo, restoreLayout,
      expandInternalEntities, showFullPathOnFrame, lang,
      wxGetApp().getAvailableTranslations(), wxID_ANY, title));
...
void MyFrame::OnGoto(wxCommandEvent &WXUNUSED(event)) {
  ...
  wxTextEntryDialog *dlg =
      new wxTextEntryDialog(this, _("Enter line number:"), _("Go To"));
...
void MyFrame::OnFindReplace(wxCommandEvent &WXUNUSED(event)) {
  ...
  findDialog = std::auto_ptr< wxFindReplaceDialog >(new wxFindReplaceDialog(
      this, &findData, _("Find and Replace"), wxFR_REPLACEDIALOG));
  findDialog->Show();
}
...
void MyFrame::OnGlobalReplace(wxCommandEvent &event) {
  ...
  std::auto_ptr< GlobalReplaceDialog > grd(new GlobalReplaceDialog(
      ...
      flags & wxFR_MATCHCASE, globalReplaceAllDocuments, findRegex));
  ...
  for (int i = 0; i < pageCount; ++i) {
    ...
    if (!findRegex) {
      ...
      globalMatchCount += Replace::run(bufferUtf8, findUtf8, replaceUtf8,
                                       flags & wxFR_MATCHCASE);
...
void MyFrame::OnNew(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (wxDirExists(templateDir)) {
    ...
    wxSingleChoiceDialog scd(this, _("Choose a document type:"),
                             _("New Document"), templateArray);
  ...
  wxString documentContents =
      wxString(buffer.c_str(), wxConvUTF8, buffer.size());

  newDocument(buffer, templateFile);
}

void MyFrame::newDocument(const wxString &s, const wxString &path,
                          bool canSave) {
  std::string bufferUtf8 = (const char *)s.mb_str(wxConvUTF8);
  newDocument(bufferUtf8, path, canSave);
}

void MyFrame::newDocument(const std::string &s, const wxString &path,
                          bool canSave) {
  XmlDoc *doc;
...

  {
    ...
    doc = (s.empty())
              ? new XmlDoc(mainBook, properties, &protectTags, visibilityState,
                           FILE_TYPE_XML, wxID_ANY, NULL,
                           0 // new: NULL pointer leads to default document
                           )
              : new XmlDoc(mainBook, properties, &protectTags, visibilityState,
                           FILE_TYPE_XML, wxID_ANY,
                           s.c_str(), // modified
                           s.size(),  // new
                           path, auxPath);
...
void MyFrame::OnOpen(wxCommandEvent &event) {
  ...
  wxFileDialog fd(this, (largeFile) ? _("Open Large Document") : _("Open"),
                  defaultDir, wxEmptyString, FILE_FILTER,
#if wxCHECK_VERSION(2, 9, 0)
...
bool MyFrame::openFile(const wxString &file, bool largeFile) {
  ...
  */
  ...
  if (encoding == "UTF-8" || encoding == "utf-8" || encoding == "US-ASCII" ||
    ...
    wxString wideEncoding =
        wxString(encoding.c_str(), wxConvLocal, encoding.size());
    iconv_t cd = iconv_open("UTF-8", encoding.c_str());
    if (cd == (iconv_t)-1) {
      wxString message;
      message.Printf(_("Cannot open %s: unknown encoding %s"), fileName.c_str(),
                     wideEncoding.c_str());
      messagePane(message, CONST_STOP);
      return false;
    };
    ...
    if (encoding == "ISO-8859-1" || encoding == "UTF-16" ||
        encoding == "UTF-16BE" || encoding == "UTF-16LE") {
      iconvLenMultiplier = 2;
    } else if (encoding == "UTF-32" || encoding == "UTF-32BE" ||
               encoding == "UTF-32LE") {
    ...
    if ((((size_t)-1) - 1) / iconvLenMultiplier < docBufferLen ||
      ...
      wxString message;
      message.Printf(_("Cannot open %s: out of memory"), fileName.c_str());
      messagePane(message, CONST_STOP);
      statusProgress(wxEmptyString);
      return false;
    }
    finalBuffer = buffer =
        iconvBuffer.data(); // buffer will be incremented by iconv
    nconv = reinterpret_cast< universal_iconv & >(iconv)(
        cd, &docBuffer, &docBufferLeft, &buffer, &iconvBufferLeft);
    ...
    *buffer = '\0';
    ...
    if (nconv == (size_t)-1) {
      ...
      message.Printf(_("Cannot open %s: conversion from encoding %s failed"),
                     fileName.c_str(), wideEncoding.c_str());
      messagePane(message, CONST_STOP);
      return false;
    }
  ...
  statusProgress(_("Creating document view..."));
  {
    ...
    doc = new XmlDoc(mainBook, (largeFile) ? largeFileProperties : properties,
                     &protectTags, visibilityState,
                     (!binaryfile.getDataLen()) ? FILE_TYPE_XML : type,
                     wxID_ANY, finalBuffer, finalBufferLen, fileName, auxPath);
#ifdef __WXMSW__
  ...
  if (!isUtf8 && finalBufferLen > 5 && finalBuffer[0] == '<' &&
      finalBuffer[1] == '?' && finalBuffer[2] == 'x' && finalBuffer[3] == 'm' &&
      finalBuffer[4] == 'l') {
  ...
  if (!largeFile && (properties.validateAsYouType && doc->getGrammarFound())) {
    ...
    // doc->backgroundValidate ( finalBuffer,
    // doc->getFullFileName().mb_str(wxConvUTF8), finalBufferLen );
    doc->backgroundValidate();
    statusProgress(wxEmptyString);
  }
...
std::string MyFrame::getApproximateEncoding(char *docBuffer,
  ...
  // grab first line
  for (i = 0, it = docBuffer; i < docBufferLen && *it != '\n' && i < BUFSIZ;
       i++, it++) {
    if (*it)
      line += *it;
  }
  ...
  std::pair< int, int > limits =
      XmlEncodingHandler::getEncodingValueLimits(line);
...
void MyFrame::OnSpelling(wxCommandEvent &event) {
  ...
  auto_ptr< StyleDialog > sd(new StyleDialog(
      this, wxICON(appicon), rawBufferUtf8, doc->getShortFileName(), ruleSetDir,
      filterDir, (type == ID_TYPE_SPELL) ? dictionaryPreset : ruleSetPreset,
      filterPreset,
#ifdef __WXMSW__
      aspellDataPath, aspellDictPath,
#endif
      type, (success) ? false : true, stylePosition, styleSize));
...
void MyFrame::OnSave(wxCommandEvent &event) { save(); }
...
void MyFrame::OnReload(wxCommandEvent &event) { reloadTab(); }
...
void MyFrame::OnSaveAs(wxCommandEvent &event) { saveAs(); }
...
void MyFrame::saveAs() {
  ...
  auto_ptr< wxFileDialog > fd(
      new wxFileDialog(this, _("Save As"), defaultDir, defaultFile, FILE_FILTER,
#if wxCHECK_VERSION(2, 9, 0)
                       wxFD_SAVE | wxFD_OVERWRITE_PROMPT));
#else
                       wxSAVE | wxOVERWRITE_PROMPT));
  ...
  if (openFileSet.count(path) && path != doc->getFullFileName()) {
    wxString message;
...
void MyFrame::OnValidateRelaxNG(wxCommandEvent &event) {
  ...
  AssociateDialog ad(this, _("Select RELAX NG grammar"), _("Choose a file:"),
                     _("RELAX NG grammar"), _T ( "*.*" ), lastRelaxNGSchema,
                     &mLastDir);
...
void MyFrame::validateRelaxNG(
    XmlDoc *doc, const wxString &schemaUrl,
...
void MyFrame::OnValidateSchema(wxCommandEvent &event) {
  ...
  if (validator->validateMemory(utf8Buffer.c_str(), utf8Buffer.size(),
                                fileName)) {
    message.Printf(_("%s is valid"), doc->getShortFileName().c_str());
  ...
  if (severity != CONST_INFO) {
    ...
    int cursorPos = doc->PositionFromLine(posPair.first - 1);
    doc->SetSelection(cursorPos, cursorPos);
...
void MyFrame::OnCreateSchema(wxCommandEvent &event) {
  ...
  const wxString &schema =
      gen.generate(type, doc->getFullFileName(), rawBufferUtf8.c_str(),
                   rawBufferUtf8.size(), _T ( "UTF-8" ));
  if (schema.IsEmpty()) {
...
void MyFrame::OnDtd2Schema(wxCommandEvent &event) {
  ...
  wxFileDialog fd(this, _("Please select a DTD file"), wxEmptyString,
                  wxEmptyString,
                  _T ( "DTD files (*.dtd)|*.dtd|All files (*.*)|*.*" ), style);
...
void MyFrame::OnXPath(wxCommandEvent &event) {
  ...
  auto_ptr< wxTextEntryDialog > dlg(new wxTextEntryDialog(
      this, _("Enter XPath:"), _("Evaluate XPath"), xpathExpression));
  ...
  auto_ptr< WrapLibxml > wl(new WrapLibxml(libxmlNetAccess));
  bool success = wl->xpath(xpathExpression, utf8Buffer, doc->getFullFileName());
...
void MyFrame::OnXslt(wxCommandEvent &event) {
  ...
  if (id == ID_XSLT) {
    ...
    if (!wxFileExists(path)) {
      ...
      AssociateDialog ad(this, _("Select stylesheet"), _("Choose a file:"),
                         _("XSLT stylesheet"), _T ( "*.xsl;*.xslt" ),
                         lastXslStylesheet, &mLastDir);
      if (ad.ShowModal() != wxID_OK)
        return;
    ...
    case ID_XSLT_DOCBOOK_HTML:
      path = applicationDir + sep + _T ( "docbook" ) + sep + _T ( "html" ) +
             sep + _T ( "docbook.xsl" );
      break;
    case ID_XSLT_DOCBOOK_XHTML:
      path = applicationDir + sep + _T ( "docbook" ) + sep + _T ( "xhtml" ) +
             sep + _T ( "docbook.xsl" );
      break;
    default:
      break;
...
void MyFrame::OnPrettyPrint(wxCommandEvent &event) {
  ...
  if (rawBufferUtf8.empty())
    messagePane(_("Pretty-print unsuccessful: output document empty"),
                CONST_STOP);
  else {
...
void MyFrame::OnEncoding(wxCommandEvent &event) {
  ...
  const static wxString encodings[] = {_T ( "UTF-8" ),      _T ( "UTF-16" ),
                                       _T ( "UTF-16LE" ),   _T ( "UTF-16BE" ),
                                       _T ( "ISO-8859-1" ), _T ( "US-ASCII" )};
  wxSingleChoiceDialog scd(this, _("Choose an encoding:"), _("Encoding"),
  ...
  std::auto_ptr< XmlUtf8Reader > xur(
      new XmlUtf8Reader(false, expandInternalEntities, output.GetDataLen()));
...
void MyFrame::OnDownloadSource(wxCommandEvent &event) {
  wxLaunchDefaultBrowser(
      _T ( "http://sourceforge.net/p/xml-copy-editor/code/" ));
}
...
void MyFrame::findAgain(wxString s, int flags) {
  ...
  {
    doc->SetCurrentPos((doc->GetSelectionStart()) ? doc->GetSelectionStart() - 1
                                                  : 0);
    doc->SearchAnchor();
...
bool MyFrame::closeActiveDocument() {
  ...
  int selection = mainBook->GetSelection();
  if (selection == -1 ||
      !mainBook->GetPageCount()) // GetPageCount needed for wxAuiNotebook
    return false;
  ...
  // workaround -- wxAuiNotebook: send virtual close event? DeletePage doesn't
  // generate one
  ...
  // apparently fixed betw. 2.8.0 and 2.8.6, so from v. 1.1.0.3, BUT this is
  // once more: 1.1.0.5: reverted to above workaround - no confirmation dialog?
  // watch memory usage for DeletePage call
  // mainBook->DeletePage ( selection );
  // return ( !deletePageVetoed );
}

bool MyFrame::saveFile(XmlDoc *doc, wxString &fileName,
                       bool checkLastModified) {
  if (!doc)
    return false;
  ...
  if (checkLastModified) {
    ...
    if (fn.IsOk()) {
      ...
      if (!myLastModified.IsEqualTo(doc->getLastModified())) {
        int choice = wxMessageBox(_("File has been modified by another "
  ...
  try {
    ...
    if (doc->getType() == FILE_TYPE_BINARY) {
      ...
      if (encoding == "UTF-16" || encoding == "UTF-16LE" ||
        ...
        if (cd == (iconv_t)-1) {
          ...
          nconv = reinterpret_cast< universal_iconv & >(iconv)(
              cd, &utf8BufferPtr, &utf8BufferLeft, &iconvBuffer,
              &iconvBufferLeft);
          ...
          if (nconv == (size_t)-1) // conversion failed
          {
            ...
            if (success) {
              bytes = utf8Buffer.size();
              wxString message;
              message.Printf(_("%s saved in default encoding UTF-8: conversion "
                               "to %s failed"),
                             fileName.c_str(), wideEncoding.c_str());
              messagePane(message, CONST_WARNING);
            } else {
            ...
            std::ofstream ofs(fileNameLocal.c_str(),
      ...
      {
        auto_ptr< WrapLibxml > wl(new WrapLibxml(libxmlNetAccess));
        int result = wl->saveEncoding(utf8Buffer, doc->getFullFileName(),
                                      fileName, NULL, wideEncoding);
        if (result == -1) {
          success = saveRawUtf8(fileNameLocal, utf8Buffer, false, isXml);
          if (success) {
            wxString wideError = wl->getLastError();
            ...
            msg.Printf(_("Cannot save document in %s: %s (saved in default "
                         "encoding UTF-8)"),
    ...
    if (encoding != "UTF-8") {
      int answer = wxMessageBox(
          ...
          _("Out of memory"), wxYES_NO | wxCANCEL | wxICON_QUESTION, this);
      ...
      if (success) {
        bytes = utf8Buffer.size();
        wxString message;
        message.Printf(_("%s saved in default encoding UTF-8"),
                       fileName.c_str());
  ...
  if (properties.validateAsYouType && isXml) {
    doc->clearErrorIndicators();
    // doc->backgroundValidate ( utf8Buffer.c_str(),
    // doc->getFullFileName().mb_str(wxConvUTF8), utf8Buffer.size() );
    doc->backgroundValidate();
  }
...
bool MyFrame::saveRawUtf8(const std::string &fileNameLocal,
                          std::string &bufferUtf8, bool ignoreEncoding,
                          bool isXml) {
...
void MyFrame::displaySavedStatus(int bytes) {
  ...
  msg.Printf(_("%g %s saved"), result, unit.c_str());
  statusProgress(msg);
}

bool MyFrame::getHandleCommandLineFlag() { return handleCommandLineFlag; }
...
wxMenuBar *MyFrame::getMenuBar() {
  ...
  wxMenuItem *pasteNewDocumentItem =
      new wxMenuItem(editMenu, ID_PASTE_NEW_DOCUMENT,
                     _("P&aste As New Document"), _("Paste As New Document"));
  ...
  wxMenuItem *findAgainItem = new wxMenuItem(
      editMenu, ID_FIND_AGAIN, _("F&ind Again\tF3"), _("Find Again"));
  findAgainItem->SetBitmap(wxNullBitmap);

  wxMenuItem *replaceItem = new wxMenuItem(
      editMenu, ID_REPLACE, _("&Replace...\tCtrl+R"), _("Replace..."));
  replaceItem->SetBitmap(wxNullBitmap);

  wxMenuItem *globalReplaceItem = new wxMenuItem(
      editMenu, ID_GLOBAL_REPLACE, _("&Global Replace...\tCtrl+Shift+R"),
      _("Global Replace..."));
  ...
  wxMenuItem *commentItem =
      new wxMenuItem(editMenu, ID_TOGGLE_COMMENT, _("&Toggle Comment\tCtrl+/"),
                     _("Toggle Comment"));
...
#ifndef __WXMSW__
  wxMenuItem *preferencesItem = new wxMenuItem(
      editMenu, ID_OPTIONS, _("Pr&eferences..."), _("Preferences..."));
  editMenu->AppendSeparator();
  ...
  fontSizeMenu->Append(ID_FONT_LARGER, _("Increase\tCtrl+U"), _("Increase"));
  fontSizeMenu->Append(ID_FONT_SMALLER, _("Decrease\tCtrl+D"), _("Decrease"));
  fontSizeMenu->AppendSeparator();
  fontSizeMenu->Append(ID_FONT_NORMAL, _("Normal\tCtrl+0"), _("Normal"));

  // color scheme menu
  colorSchemeMenu = new wxMenu;
  colorSchemeMenu->AppendRadioItem(ID_COLOR_SCHEME_DEFAULT, _("&Default"),
                                   _("Default"));
  colorSchemeMenu->AppendRadioItem(ID_COLOR_SCHEME_REDUCED_GLARE,
                                   _("&Blue background, white text"),
                                   _("Blue background, white text"));
  colorSchemeMenu->AppendRadioItem(ID_COLOR_SCHEME_DEFAULT_BACKGROUND,
                                   _("&Light"), _("Light"));
  colorSchemeMenu->AppendRadioItem(ID_COLOR_SCHEME_NONE, _("&None"), _("None"));
  ...
  */
  ...
  viewMenu->Append(ID_PREVIOUS_DOCUMENT, _("&Previous Document\tCtrl+PgUp"),
                   _("Previous Document"));
  viewMenu->Append(ID_NEXT_DOCUMENT, _("&Next Document\tCtrl+PgDn"),
                   _("Next Document"));
  ...
  viewMenu->AppendRadioItem(ID_SHOW_TAGS,
                            _("&Show Tags and Attributes\tCtrl+T"),
                            _("Show Tags and Attributes"));
  viewMenu->AppendRadioItem(ID_HIDE_ATTRIBUTES,
                            _("&Hide Attributes Only\tCtrl+Shift+A"),
                            _("Hide Attributes Only"));
  viewMenu->AppendRadioItem(ID_HIDE_TAGS,
                            _("H&ide Tags and Attributes\tCtrl+Shift+T"),
                            _("Hide Tags and Attributes"));
  ...
  viewMenu->AppendSeparator();
  viewMenu->Append(ID_TOGGLE_FOLD, _("&Toggle Fold\tCtrl+Alt+T"),
                   _("Toggle Fold"));
  viewMenu->Append(ID_FOLD_ALL, _("&Fold Tags\tCtrl+Shift+F"), _("Fold Tags"));
  viewMenu->Append(ID_UNFOLD_ALL, _("&Unfold Tags\tCtrl+Shift+U"),
                   _T ( "Unfold Tags" ));
  viewMenu->AppendSeparator();
  viewMenu->AppendCheckItem(ID_WRAP_WORDS, _("&Wrap Words"),
                            _T ( "Wrap Words" ));
  ...
  viewMenu->AppendCheckItem(ID_LOCATION_PANE_VISIBLE,
                            _("S&how Current Element Pane"),
                            _("Show Current Element Pane"));
  viewMenu->Check(ID_LOCATION_PANE_VISIBLE, false);
#ifndef __WXOSX__
  viewMenu->AppendCheckItem(ID_TOOLBAR_VISIBLE, _("Sh&ow Toolbar"),
                            _("Show Toolbar"));
  viewMenu->Check(ID_TOOLBAR_VISIBLE, toolbarVisible);
#endif
  viewMenu->Append(ID_CLOSE_MESSAGE_PANE, _("C&lose Message Pane\tAlt+C"),
                   _("Close Message Pane"));
  viewMenu->Append(ID_CLOSE_FIND_REPLACE_PANE, _("Close Find/&Replace Pane"),
                   _("Close Find/Replace Pane"));
  viewMenu->Append(ID_CLOSE_COMMAND_PANE, _("Close Co&mmand Pane"),
                   _("Close Command Pane"));

  // insert menu
  wxMenu *insertMenu = new wxMenu;
  insertMenu->Append(ID_INSERT_CHILD, _("&Element...\tCtrl+I"),
                     _("Element..."));
  insertMenu->Append(ID_INSERT_SIBLING, _("&Sibling...\tCtrl+Shift+I"),
                     _("Sibling..."));
  ...
  */
  validationMenu->Append(ID_VALIDATE_W3C_SCHEMA, _("&DTD/XML Schema\tF5"),
                         _("DTD/XML Schema"));
  validationMenu->AppendSeparator();
  validationMenu->Append(ID_VALIDATE_RELAX_NG, _("&RELAX NG...\tF6"),
                         _("RELAX NG..."));

  wxMenu *associateMenu = new wxMenu;
  associateMenu->Append(ID_ASSOCIATE_DTD_PUBLIC, _("&Public DTD..."),
                        _("Public DTD..."));
  associateMenu->Append(ID_ASSOCIATE_DTD_SYSTEM, _("&System DTD..."),
                        _("System DTD..."));
  associateMenu->Append(ID_ASSOCIATE_W3C_SCHEMA, _("&XML Schema..."),
                        _("XML Schema..."));
  associateMenu->Append(ID_ASSOCIATE_XSL, _("XS&LT stylesheet..."),
                        _("XSLT stylesheet..."));
  ...
  */
  ...
  xmlMenu->Append(ID_CHECK_WELLFORMED, _("&Check Well-formedness\tF2"),
                  _("Check Well-formedness"));
  xmlMenu->Append(wxID_ANY, _("&Validate"), validationMenu);
  xmlMenu->Append(ID_CREATE_SCHEMA, _("Create &Schema...\tF10"),
                  _("Create schema..."));
  xmlMenu->Append(ID_DTD_TO_SCHEMA, _("DTD -> Schema..."),
                  _("DTD -> Schema..."));
  xmlMenu->AppendSeparator();
  xmlMenu->Append(wxID_ANY, _("&Associate"), associateMenu);
  xmlMenu->AppendSeparator();
  xmlMenu->Append(ID_XSLT, _("&XSL Transform...\tF8"), _("XSL Transform..."));
  xmlMenu->Append(ID_XPATH, _("&Evaluate XPath...\tF9"),
                  _("Evaluate XPath..."));
  xmlMenu->Append(ID_COPY_XPATH, _("Copy &The Current XPath"),
                  _("Copy The Current XPath"));

  xmlMenu->AppendSeparator();
  xmlMenu->Append(ID_PRETTYPRINT, _("&Pretty-print\tF11"), _("Pretty-print"));
  xmlMenu->AppendSeparator();
  xmlMenu->AppendCheckItem(ID_PROTECT_TAGS, _("&Lock Tags\tCtrl+L"),
                           _("Lock Tags"));
  xmlMenu->Check(ID_PROTECT_TAGS, protectTags);
  xmlMenu->AppendSeparator();
  xmlMenu->Append(ID_ENCODING, _("E&ncoding..."), _("Encoding..."));

  // tools menu
  wxMenu *toolsMenu = new wxMenu;

  wxMenuItem *spellingItem = new wxMenuItem(
      toolsMenu, ID_SPELL, _("&Spelling...\tF7"), _("Spelling..."));
  spellingItem->SetBitmap(spelling16Bitmap);

  wxMenuItem *styleItem = new wxMenuItem(
      toolsMenu, ID_STYLE, _("&Style...\tShift+F7"), _("Style..."));
  styleItem->SetBitmap(wxNullBitmap);

  wxMenuItem *wordCountItem = new wxMenuItem(toolsMenu, ID_WORD_COUNT,
                                             _("&Word Count"), _("Word Count"));
  wordCountItem->SetBitmap(wxNullBitmap);

  wxMenuItem *commandItem = new wxMenuItem(
      toolsMenu, ID_COMMAND, _("&Command\tCtrl+Alt+C"), _("Command"));
  ...
  wxMenuItem *optionsItem =
      new wxMenuItem(toolsMenu, ID_OPTIONS, _("&Options..."), _("Options..."));
  ...
  wxMenuItem *helpItem = new wxMenuItem(
      helpMenu, wxID_HELP, _("&XML Copy Editor Help\tF1"), _("Help"));
  helpItem->SetBitmap(helpBitmap);

  wxMenuItem *homeItem =
      new wxMenuItem(helpMenu, ID_HOME, _("&Home Page"), _("Home Page"));
  ...
  wxMenuItem *aboutItem = new wxMenuItem(
      helpMenu, wxID_ABOUT, _("&About XML Copy Editor"), _("About"));
  aboutItem->SetBitmap(wxNullBitmap);
  wxMenuItem *downloadSourceItem = new wxMenuItem(
      helpMenu, ID_DOWNLOAD_SOURCE, _("&Browse Source"), _("Browse Source"));
...
void MyFrame::updateFileMenu(bool deleteExisting) {
  ...
  wxMenuItem *openLargeFileItem = new wxMenuItem(
      fileMenu, ID_OPEN_LARGE_FILE, _("O&pen Large Document...\tCtrl+Shift+O"),
      _("Open Large Document..."));
  ...
  wxMenuItem *saveAsItem = new wxMenuItem(
      fileMenu, wxID_SAVEAS, _("S&ave As...\tF12"), _("Save As..."));
  saveAsItem->SetBitmap(wxNullBitmap);
#ifdef __WXMSW__
  wxMenuItem *exportItem = new wxMenuItem(
      fileMenu, ID_EXPORT, _("&DAISY Export..."), _("DAISY Export..."));
  ...
  wxMenuItem *printSetupItem = new wxMenuItem(
      fileMenu, ID_PRINT_SETUP, _("Pa&ge Setup..."), _("Page Setup..."));
  printSetupItem->SetBitmap(wxNullBitmap);
  wxMenuItem *printPreviewItem =
      new wxMenuItem(fileMenu, ID_PRINT_PREVIEW, _("Pr&int Preview..."),
                     _("Print Preview..."));
...
wxToolBar *MyFrame::getToolBar() {
  wxToolBar *toolBar =
      new wxToolBar(this, ID_TOOLBAR, wxDefaultPosition, wxDefaultSize,
                    wxTB_FLAT | wxTB_HORIZONTAL | wxTB_DOCKABLE);
  int w, h;
  w = saveBitmap.GetWidth(), h = saveBitmap.GetHeight();
  toolBar->SetToolBitmapSize(wxSize(w, h));

  toolBar->AddTool(wxID_NEW, _("New"), newBitmap, _("New"));
  toolBar->AddTool(wxID_OPEN, _("Open"), openBitmap, _("Open"));
  toolBar->AddTool(wxID_SAVE, _("Save"), saveBitmap, wxNullBitmap,
                   wxITEM_NORMAL, _("Save"));
#ifndef __WXOSX__
  toolBar->AddTool(ID_PRINT, _("Print"), printBitmap, wxNullBitmap,
                   wxITEM_NORMAL, _("Print"));
  toolBar->AddTool(ID_CHECK_WELLFORMED, _("Check Well-formedness"),
                   checkWellformedBitmap, wxNullBitmap, wxITEM_NORMAL,
                   _("Check Well-formedness"));
  toolBar->AddTool(ID_VALIDATE_W3C_SCHEMA, _("Validate"), checkValidBitmap,
                   wxNullBitmap, wxITEM_NORMAL, _("Validate"));
  toolBar->AddTool(ID_BROWSER, _("Browser"), internetBitmap, wxNullBitmap,
                   wxITEM_NORMAL, _("Browser"));
  toolBar->AddTool(ID_SPELL, _("Spelling"), spellingBitmap, wxNullBitmap,
                   wxITEM_NORMAL, _("Spelling"));
  toolBar->AddCheckTool(ID_PROTECT_TAGS, _("Lock Tags"), hyperlinkBitmap,
                        wxNullBitmap, _("Lock Tags"));
  toolBar->ToggleTool(ID_PROTECT_TAGS, protectTags);
#endif
...
void MyFrame::addSafeSeparator(wxToolBar *toolBar) {
  wxStaticText *staticControl =
      new wxStaticText(toolBar, wxID_ANY, _T ( "  " ));
...
void MyFrame::documentOk(const wxString &status) {
  ...
  if ((doc = getActiveDocument()) == NULL)
    return;
  wxString message;
  message.Printf(_("%s is %s"), doc->getShortFileName().c_str(),
                 status.c_str());
  messagePane(message, CONST_INFO);
}
...
void MyFrame::modifiedMessage() {
  messagePane(_("Document has been modified: save or discard changes"),
              CONST_STOP);
}
...
long MyFrame::getNotebookStyleMask() {
  ...
  if (notebookStyleMenu->IsChecked(ID_NOTEBOOK_STYLE_FLAT))
    ...
    return wxFNB_BACKGROUND_GRADIENT | wxFNB_VC8 |
  wxFNB_MOUSE_MIDDLE_CLOSES_TABS | wxFNB_X_ON_TAB | wxFNB_DROPDOWN_TABS_LIST |
  wxFNB_NO_NAV_BUTTONS; else return wxFNB_BACKGROUND_GRADIENT | wxFNB_VC8 |
  wxFNB_MOUSE_MIDDLE_CLOSES_TABS | wxFNB_COLORFUL_TABS | wxFNB_X_ON_TAB |
  wxFNB_DROPDOWN_TABS_LIST | wxFNB_NO_NAV_BUTTONS;
    //wxFNB_BACKGROUND_GRADIENT | wxFNB_VC8 | wxFNB_MOUSE_MIDDLE_CLOSES_TABS |
  wxFNB_COLORFUL_TABS;
  */
  return 0;
}
...
bool MyFrame::isSpecialFileType(const wxString &fileName) {
  ...
  return (fileNameLocalLC.find(".dtd") != std::string::npos ||
...
void MyFrame::updatePaths() {
  ...
  xslDtdPath = applicationDir + sep + _T ( "dtd" ) + sep + _T ( "xslt10.dtd" );
  rssDtdPath = applicationDir + sep + _T ( "dtd" ) + sep + _T ( "rss2.dtd" );
  xtmDtdPath = applicationDir + sep + _T ( "dtd" ) + sep + _T ( "xtm1.dtd" );
  lzxDtdPath = applicationDir + sep + _T ( "dtd" ) + sep + _T ( "lzx.dtd" );
  xliffDtdPath = applicationDir + sep + _T ( "dtd" ) + sep + _T ( "xliff.dtd" );
  aspellDataPath = applicationDir + sep + _T ( "aspell" ) + sep + _T ( "data" );
  aspellDictPath = applicationDir + sep + _T ( "aspell" ) + sep + _T ( "dict" );
}
...
void MyFrame::OnAssociate(wxCommandEvent &event) {
  ...
  case ID_ASSOCIATE_DTD_PUBLIC:
    ...
    defaultaux = info.mDtdFile.empty() ? lastDtdPublicAux : info.mDtdFile;
    break;
  ...
  case ID_ASSOCIATE_XSL:
    ...
    defaulturl = info.mXslFile.empty() ? lastXslStylesheet : info.mXslFile;
    defaultaux = _T ( "" );
    break;
  default:
    return;
  ...
  if (!wellformedparser.parse(utf8Buffer)) {
    wxString message;
    message.Printf(_("Cannot associate %s: %s"), type.c_str(),
  ...
  bool auxiliaryBox = (id == ID_ASSOCIATE_DTD_PUBLIC);
  ...
  AssociateDialog ad(this, title, label, type, extension, defaulturl, &mLastDir,
                     auxiliaryBox, auxiliaryLabel, defaultaux);
  if (ad.ShowModal() != wxID_OK)
    return;
...
void MyFrame::OnWordCount(wxCommandEvent &event) {
  ...
  msg.Printf(wxPLURAL("%s contains %i word", "%s contains %i words", count),
...
/*
        ...
        saveBitmap.LoadFile ( xpmDir + _T ( "stock_save.xpm" ),
wxBITMAP_TYPE_XPM );
...
*/
  ...
  spellingBitmap =
      wxArtProvider::GetBitmap(_T ( "gtk-spell-check" ), wxART_TOOLBAR);

  // no stock icons for the following
  internetBitmap.LoadFile(pngDir + _T ( "stock_internet.png" ),
                          wxBITMAP_TYPE_PNG);
  hyperlinkBitmap.LoadFile(pngDir + _T ( "stock_hyperlink.png" ),
                           wxBITMAP_TYPE_PNG);
  checkWellformedBitmap.LoadFile(pngDir + _T ( "stock_calc-accept.png" ),
                                 wxBITMAP_TYPE_PNG);
  checkValidBitmap.LoadFile(pngDir + _T ( "stock_calc-accept-green.png" ),
                            wxBITMAP_TYPE_PNG);
#endif
...
void MyFrame::OnKeyPressed(wxKeyEvent &event) { event.Skip(); }
...
void MyFrame::setStrictScrolling(bool b) {
  ...
  if (!doc)
    return;
  doc->SetYCaretPolicy((b) ? (wxSTC_CARET_STRICT | wxSTC_CARET_SLOP)
                           : wxSTC_CARET_EVEN,
                       (b) ? 10 : 0);
}
...
#undef CHECK
  ...
  msg << wxTextFile::GetEOL() << wxTextFile::GetEOL()
#ifdef __WXMSW__
