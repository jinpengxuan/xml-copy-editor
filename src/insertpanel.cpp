...
InsertPanel::InsertPanel(wxWindow *parentWindowParameter, int id,
                         int typeParameter)
    : wxPanel(parentWindowParameter, id), type(typeParameter), edit(0),
      list(0) {
  ...
  edit = new wxTextCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition,
  ...
  list = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0,
                       NULL, wxLB_SORT | wxLB_HSCROLL);
...
void InsertPanel::update(XmlDoc *docParameter, const wxString &parentParameter,
                         const wxString &grandparentParameter, bool forced) {
  doc = docParameter;
  ...
  list->Clear();
  if (parent.empty() ||
      ((type == INSERT_PANEL_TYPE_SIBLING) && grandparent.empty())) {
    list->Show(false);
    return;
  }
...
void InsertPanel::adjustSize() {
  ...
  wxSize listSize = wxSize(clientSize.GetWidth(),
                           clientSize.GetHeight() - editSize.GetHeight());
