...
const wxString &Dtd2Schema::convert(const wxString &dtdFile) {
  ...
  if (targetNameSpace != NULL && *targetNameSpace != 0) {
    ...
    if (mTargetNameSpace.empty())
      ...
      mErrors << wxString::Format(
          _("Target namespace is redefined: %s -> %s[br]"), targetNS.c_str(),
          mTargetNameSpace.c_str());
  }
  if (!mTargetNameSpace.empty())
    mSchema << wxTextFile::GetEOL() << _T("    targetNamespace=\"")
            << mTargetNameSpace << _T("\"") << wxTextFile::GetEOL()
  ...
  for (itr = mNameSpaceMap.begin(); itr != mNameSpaceMap.end(); ++itr) {
    mSchema << wxTextFile::GetEOL() << _T("    xmlns:") << itr->first
            << _T("=\"") << itr->second << _T("\"");
    importNS << _T("  <xs:import namespace=\"")
             << (itr->second.empty() ? itr->first : itr->second) << _T("\"/>")
             << wxTextFile::GetEOL();
  }
  mSchema << _T(">") << wxTextFile::GetEOL() << importNS << wxTextFile::GetEOL()
          << elements;
...
wxString Dtd2Schema::convertElement(const DTDElementDecl &element,
  ...
  if (!element.hasAttDefs()) {
    ...
    if (pcdata) {
      schema << _T("\" type=\"xs:string\"/>") << wxTextFile::GetEOL()
             << wxTextFile::GetEOL();
      return schema;
    }
  }
  ...
  for (i = 0; i < count; i++) {
    schema << convertAttribute(attList.getAttDef(i), nIndent + 2, elementName);
  }
  ...
  XmlSchemaGenerator::addIndent(schema, nIndent);
  schema << _T("</xs:element>") << wxTextFile::GetEOL() << wxTextFile::GetEOL();

  return schema;
}

wxString Dtd2Schema::convertContent(const ContentSpecNode *content,
                                    size_t nIndent,
  ...
  case ContentSpecNode::Sequence:
    ...
    XmlSchemaGenerator::addIndent(schema, nIndent);
    schema << _T("<xs:sequence") << convertOccurrence(content) << _T(">")
           << wxTextFile::GetEOL();
    XmlSchemaGenerator::addIndent(suffix, nIndent);
    suffix << _T("</xs:sequence>") << wxTextFile::GetEOL();
    break;
  ...
  default: {
    ...
    schema << _T("<xs:element ref=\"") << name << _T("\"")
           << convertOccurrence(content) << _T("/>") << wxTextFile::GetEOL();

    size_t index = name.Index(':');
    if (index != wxString::npos) { // Validate the name space
      wxString nameSpace = name.Left(index);
      if (mNameSpaceMap.find(nameSpace) == mNameSpaceMap.end()) {
        if (nameSpace != _T("xml"))
          mErrors << wxString::Format(_("Unknown namespace: %s[br]"),
                                      nameSpace.c_str());
...
wxString Dtd2Schema::convertAttribute(const XMLAttDef &att, size_t nIndent,
  ...
  if (name == _T("xmlns")) {
    ...
    if (mTargetNameSpace.empty())
      ...
      mErrors << wxString::Format(_("Ignored namespace of %s: %s[br]"),
  ...
  if (index != wxString::npos) {
    ...
    if (ns == _T("xmlns")) // A new name space is defined
    {
      ...
      if (mNameSpaceMap[ns].empty())
        ...
        mErrors << wxString::Format(_("Namespace redefined: %s -> %s[br]"),
                                    mNameSpaceMap[ns].c_str(), url.c_str());
      return wxEmptyString;
    } else if (mNameSpaceMap.find(ns) == mNameSpaceMap.end()) {
      if (ns != _T("xml"))
        mErrors << wxString::Format(_("Unknown namespace: %s[br]"), ns.c_str());
  ...
  {
    schema << convertAttValue(att) << _T(">") << wxTextFile::GetEOL()
           << convertAttType(att, nIndent + 1);
    XmlSchemaGenerator::addIndent(schema, nIndent);
    schema << _T("</xs:attribute>") << wxTextFile::GetEOL();
  } else {
    schema << convertAttType(att, nIndent + 1) << convertAttValue(att)
...
wxString Dtd2Schema::convertAttType(const XMLAttDef &att, size_t nIndent) {
  ...
  // case XMLAttDef::Enumeration:
  {
    ...
    type << _T("<xs:restriction base=\"xs:string\">") << wxTextFile::GetEOL();

    wxStringTokenizer tokens(WrapXerces::toString(att.getEnumeration()),
                             _T(" "));
    while (tokens.HasMoreTokens()) {
      XmlSchemaGenerator::addIndent(type, nIndent + 2);
      type << _T("<xs:enumeration value=\"") << tokens.GetNextToken()
           << _T("\"/>") << wxTextFile::GetEOL();
    }
  ...
  default:
    ...
    mErrors << wxString::Format(_("Ignored attribute \"%s\"'s type: %s[br]"),
...
wxString Dtd2Schema::convertEntity(const DTDEntityDecl &entity,
                                   size_t nIndent) {
  wxString schema;
...
wxString Dtd2Schema::convertNotation(const XMLNotationDecl &notation,
                                     size_t nIndent) {
  wxString schema;
  XmlSchemaGenerator::addIndent(schema, nIndent);
  ...
  if (id != NULL)
    schema << _T("\" public=\"") << WrapXerces::toString(id);
  id = notation.getSystemId();
  if (id != NULL)
    schema << _T("\" system=\"") << WrapXerces::toString(id);
