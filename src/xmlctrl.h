...
enum VisibilityStates { SHOW_TAGS, HIDE_ATTRIBUTES, HIDE_TAGS };
...
enum BackgroundStates { BACKGROUND_STATE_NORMAL, BACKGROUND_STATE_LIGHT };
...
public:
  XmlCtrl(wxWindow *parent, const XmlCtrlProperties &propertiesParameter,
          bool *protectTagsParameter, int visibilityStateParameter = SHOW_TAGS,
          int typeParameter = FILE_TYPE_XML, wxWindowID id = wxID_ANY,
  ...
  int findNextEndTag(int pos, unsigned depth = 1, int character = '>',
                     int range = USHRT_MAX * 4);
  int findPreviousStartTag(int pos, unsigned depth = 1, int character = '<',
                           int range = USHRT_MAX * 4);
  void applyProperties(const XmlCtrlProperties &propertiesParameter,
                       bool zoomOnly = false);
  ...
  bool backgroundValidate(const char *buffer, const wxString &system,
                          size_t bufferLen);
