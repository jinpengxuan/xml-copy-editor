...
XmlShallowValidator::XmlShallowValidator(
    ...
    std::set< wxString > &entitySet, int maxLine, bool segmentOnly)
...
void XMLCALL XmlShallowValidator::start(void *data, const XML_Char *el,
                                        const XML_Char **attr) {
  ...
  if (!vd->elementMap[parent].count(element)) {
    vd->isValid = false;
    vd->positionVector.push_back(make_pair(XML_GetCurrentLineNumber(vd->p),
  ...
  while (*attr) {
    ...
    if (!attributeMap.count(currentAttribute)) {
      ...
      vd->positionVector.push_back(make_pair(
          XML_GetCurrentLineNumber(vd->p), XML_GetCurrentColumnNumber(vd->p)));
    }
...
void XMLCALL XmlShallowValidator::skippedentity(void *data,
...
bool XmlShallowValidator::isValid() { return vd->isValid; }

std::vector< std::pair< int, int > > XmlShallowValidator::getPositionVector() {
  return vd->positionVector;
}

bool XmlShallowValidator::getOverrideFailure() { return vd->overrideFailure; }
