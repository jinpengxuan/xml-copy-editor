...
MyPrintout::MyPrintout(XmlDoc *doc, const wxString &title)
    : wxPrintout(title), mDoc(doc), mMinPage(1), mMaxPage(0) {
  wxASSERT(doc);
}
...
void MyPrintout::OnPreparePrinting() {
  ...
  // Lets make a guess as to how many pages there are based on the default
  // printer.
...
void MyPrintout::OnEndPrinting() { wxPrintout::OnEndPrinting(); }

bool MyPrintout::HasPage(int nPage) {
  return (nPage >= mMinPage && nPage <= mMaxPage);
}

void MyPrintout::GetPageInfo(int *minPage, int *maxPage, int *pageFrom,
                             int *pageTo) {
  *minPage = mMinPage;
  *maxPage = mMaxPage;
  *pageFrom = mMinPage;
...
bool MyPrintout::OnPrintPage(int page) {
  ...
  mDoc->FormatRange(true, mStartPositions[page - 1], mDoc->GetLength(), dc, dc,
...
void MyPrintout::SetupDC() {
  ...
  // set user scale
  double scaleX = double(ppiPrt.x) * dcSize.x / (double(ppiScr.x) * pageSize.x);
  double scaleY = double(ppiPrt.y) * dcSize.y / (double(ppiScr.y) * pageSize.y);
...
int MyPrintout::CalcMaxPage() {
  ...
  while (start < end) {
    ...
    start = mDoc->FormatRange(false, start, end, dc, dc, mPrintRect, mPageRect);
  }
...
int MyPrintout::PrintHeader() { return 0; }

// Returns the height of the footer
int MyPrintout::PrintFooter() { return 0; }
