...
bool XmlEncodingHandler::hasDeclaration(const std::string &utf8) {
  ...
  if (len < 6 || utf8[0] != '<' || utf8[1] != '?' || utf8[2] != 'x' ||
      utf8[3] != 'm' || utf8[4] != 'l')
    return false;
  return true;
}

std::string XmlEncodingHandler::get(const std::string &utf8) {
...
bool XmlEncodingHandler::setUtf8(std::string &utf8,
                                 bool ignoreCurrentEncoding) {
  if (!hasDeclaration(utf8))
    return true;
