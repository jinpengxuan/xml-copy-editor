...
void WrapXerces::Init(bool enableNetAccess) throw() {
  ...
  public:
    ...
    ~Initializer() { XMLPlatformUtils::Terminate(); }
  } dummy;

  enableNetwork(enableNetAccess);
}

WrapXerces::WrapXerces() { catalogResolver = new XercesCatalogResolver(); }

WrapXerces::~WrapXerces() { delete catalogResolver; }
...
bool WrapXerces::validateMemory(const char *utf8Buffer, size_t len,
...
wxMemoryBuffer WrapXerces::toString(const wxString &str) {
  const static XMLCh chNull =
      '\0'; // Xerces-C crashes when the file name is NULL. We'd better return
            // something other than NULL.
  wxMemoryBuffer buffer(0);
  const size_t lenWC =
      str.length() + 1; // Plus '\0'. This is important. Otherwise we can call
                        // wxString::mb_str(getMBConv()).
  ...
  lenMB = getMBConv().FromWChar((char *)buffer.GetData(), lenMB, str.c_str(),
                                lenWC);
...
void MySAX2Handler::logError(const wxString &type, wxLogLevel level,
  ...
  mErrors << wxString::Format(
      _("%s at line %llu, column %llu: %s%s"), type.c_str(), e.getLineNumber(),
      e.getColumnNumber(), WrapXerces::toString(e.getMessage()).c_str(),
      mEOL.c_str());
...
InputSource *WrapXerces::resolveEntity(const wxString &publicId,
                                       const wxString &systemId,
                                       const wxString &fileName) {
  XercesCatalogResolver cr;
  InputSource *source =
      cr.resolveEntity((const XMLCh *)WrapXerces::toString(publicId).GetData(),
...
DOMElement *WrapXerces::getNextElementSibling(const DOMElement &element) {
#if _XERCES_VERSION >= 30100
...
DOMNode *WrapXerces::getNextLogicalSibling(const DOMNode *n) {
  // Copied from Xerces-C
  DOMNode *next = n->getNextSibling();
  // If "n" has no following sibling and its parent is an entity reference node
  // we need to continue the search through the following siblings of the entity
  // reference as these are logically siblings of the given node.
  if (!next) {
    DOMNode *parent = n->getParentNode();
    while (parent && parent->getNodeType() == DOMNode::ENTITY_REFERENCE_NODE) {
