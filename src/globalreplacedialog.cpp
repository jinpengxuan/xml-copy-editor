...
GlobalReplaceDialog::GlobalReplaceDialog(wxWindow *parent,
  ...
  findCtrl =
      new wxTextCtrl(this, ID_FIND, find, wxDefaultPosition, wxSize(400, -1));

  replaceCtrl = new wxTextCtrl(this, ID_REPLACE, replace, wxDefaultPosition,
                               wxSize(400, -1));

  matchCaseBox = new wxCheckBox(this, ID_MATCHCASE, _("&Match case"));
  matchCaseBox->SetValue(matchCase);

  regexBox = new wxCheckBox(this, ID_REGEX, _("&Regex"));
  regexBox->SetValue(regex);

  allDocumentsBox = new wxCheckBox(this, ID_ALLDOCUMENTS,
                                   _("R&eplace in all open documents"));
...
void GlobalReplaceDialog::OnOk(wxCommandEvent &e) {
  ...
  if (regex) {
    try {
      std::auto_ptr< WrapRegex > wr(
          new WrapRegex(findUtf8, matchCase, replaceUtf8));
    } catch (std::exception &e) {
      std::string pcreError(e.what());
      wxString widePcreError =
          wxString(pcreError.c_str(), wxConvUTF8, pcreError.size());
      wxMessageBox(_("Cannot compile regular expression '") +
                       findCtrl->GetValue() + _T ( "': " ) + widePcreError,
...
void GlobalReplaceDialog::OnContextHelp(wxHelpEvent &e) {
  ...
  if (id == ID_FIND)
    new wxTipWindow(
        this, _("Provides a space for you to type the text you want to find"));
  else if (id == ID_REPLACE)
    new wxTipWindow(this, _("Provides a space for you to type the text you "
                            "want to replace the text you typed in Find what"));
  else if (id == ID_MATCHCASE)
    new wxTipWindow(this, _("Finds only text with lowercase and uppercase "
                            "letters as specified in Find what"));
  else if (id == ID_ALLDOCUMENTS)
    new wxTipWindow(this, _("Extends the scope to all open documents"));
  else if (id == ID_REGEX)
    new wxTipWindow(this, _("Interprets the text specified in Find what as a "
                            "regular expression"));
  else if (id == wxID_OK)
    new wxTipWindow(this,
                    _("Finds all instances of the text specified in Find what "
                      "and replaces them with the text in Replace with"));
...
wxString GlobalReplaceDialog::getFindString() { return find; }
wxString GlobalReplaceDialog::getReplaceString() { return replace; }
bool GlobalReplaceDialog::getRegex() { return regex; }
bool GlobalReplaceDialog::getAllDocuments() { return allDocuments; }
bool GlobalReplaceDialog::getMatchCase() { return matchCase; }
