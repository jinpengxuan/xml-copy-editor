...
EVT_COMMAND(wxID_ANY, wxEVT_COMMAND_VALIDATION_COMPLETED,
            XmlCtrl::OnValidationCompleted)
...
XmlCtrl::XmlCtrl(wxWindow *parent, const XmlCtrlProperties &propertiesParameter,
                 bool *protectTagsParameter, int visibilityStateParameter,
                 int typeParameter, wxWindowID id,
                 const char *buffer, // could be NULL
                 size_t bufferLen, const wxString &basePathParameter,
                 const wxString &auxPathParameter, const wxPoint &position,
                 const wxSize &size, long style)
    : wxStyledTextCtrl(parent, id, position, size, style), type(typeParameter),
      protectTags(protectTagsParameter),
      visibilityState(visibilityStateParameter), basePath(basePathParameter),
  ...
  // position cursor
  if (type == FILE_TYPE_XML && bufferLen > 5 && buffer[0] == '<' &&
      buffer[1] == '?' && buffer[2] == 'x' && buffer[3] == 'm' &&
      buffer[4] == 'l' && GetLineCount() > 1) {
...
void XmlCtrl::OnChar(wxKeyEvent &event) {
  ...
  if ((!properties.completion || GetOvertype()) && !*protectTags) {
    event.Skip();
    return;
  }
...
void XmlCtrl::handleBackspace(wxKeyEvent &event) {
  ...
  if (GetCharAt(limitPos) == '>' &&
      (limitStyle == wxSTC_H_TAG || limitStyle == wxSTC_H_TAGUNKNOWN ||
       limitStyle == wxSTC_H_TAGEND || limitStyle == wxSTC_H_XMLSTART ||
  ...
  // entity reference
    ...
    for (; limitPos && getLexerStyleAt(limitPos) == wxSTC_H_ENTITY &&
           GetCharAt(limitPos) != '&';
         limitPos--)
      ;
    ...
    if (*protectTags) {
      SetReadOnly(true); // needed to prevent erroneous BS insertion by control
      int ret = wxMessageBox(_("Delete entity reference?"), _("Tags Locked"),
                             wxOK | wxCANCEL | wxICON_QUESTION);
      SetReadOnly(false);
      if (ret != wxOK)
        return;
...
void XmlCtrl::handleDelete(wxKeyEvent &event) {
  ...
  if (!canMoveRightAt(GetCurrentPos()) &&
    ...
    if (GetCharAt(limitPos) == '<' &&
      ...
      for (; GetCharAt(limitPos) != '>' && limitPos < GetLength(); limitPos++) {
    ...
    // entity
             ...
             getLexerStyleAt(limitPos) == wxSTC_H_ENTITY) {
      for (; getLexerStyleAt(limitPos) == wxSTC_H_ENTITY &&
             limitPos <= GetLength();
           limitPos++) {
        if (GetCharAt(limitPos) == ';')
          break;
      ...
      if (currentPos != limitPos) {
        ...
        if (*protectTags) {
          int ret =
              wxMessageBox(_("Delete entity reference?"), _("Tags Locked"),
                           wxOK | wxCANCEL | wxICON_QUESTION);
          if (ret != wxOK)
            return;
        }
...
void XmlCtrl::handleCloseAngleBracket(wxKeyEvent &event) {
  ...
  else if (GetCharAt(pos - 1) == '/' || GetCharAt(pos - 1) == ']') {
    event.Skip();
    return;
  }
  // exit condition 3 (comment/CDATA)
           ...
           (getLexerStyleAt(pos - 1) == wxSTC_H_DOUBLESTRING &&
            (GetCharAt(pos - 1) != '"')) ||
           (getLexerStyleAt(pos - 1) == wxSTC_H_SINGLESTRING &&
            (GetCharAt(pos - 1) != '\''))) {
    event.Skip();
    return;
  }
...
void XmlCtrl::handleEquals(wxKeyEvent &event) {
  ...
  int cutoff = BUFSIZ;
  for (valueSetIterator = valueSet.begin(); valueSetIterator != valueSet.end();
       ++valueSetIterator) {
    if (!(cutoff--))
      break;
    if (!choice.empty())
...
void XmlCtrl::handleSpace(wxKeyEvent &event) {
  ...
  if (style == wxSTC_H_TAG || style == wxSTC_H_TAGUNKNOWN ||
      style == wxSTC_H_ATTRIBUTEUNKNOWN || style == wxSTC_H_ATTRIBUTE) {
    proceed = true;
  }
  // space pressed after attribute value
  else if ((style == wxSTC_H_DOUBLESTRING || style == wxSTC_H_SINGLESTRING) &&
           (c == '\'' || c == '"') &&
  ...
  std::map< wxString, std::set< wxString > > &curAttMap =
      attributeMap[elementName];
...
void XmlCtrl::handleAmpersand(wxKeyEvent &event) {
  ...
  if (style != wxSTC_H_COMMENT && style != wxSTC_H_CDATA &&
...
void XmlCtrl::OnKeyPressed(wxKeyEvent &event) {
  ...
  case WXK_RIGHT:
    ...
    if (c == '<' && event.ControlDown() && !event.ShiftDown()) {
      for (iteratorPos = pos;
           iteratorPos < maxPos && GetCharAt(iteratorPos) != ' ' &&
           GetCharAt(iteratorPos) != '>' && GetCharAt(iteratorPos) != '\n';
           ++iteratorPos)
        ;
      ++iteratorPos;
  ...
  case WXK_LEFT:
    ...
    c = GetCharAt(pos - 1);
    if (c == '>' && event.ControlDown() && !event.ShiftDown()) {
      for (iteratorPos = pos - 1;
           iteratorPos > 0 && GetCharAt(iteratorPos) != '<' &&
           GetCharAt(iteratorPos) != ' ' && GetCharAt(iteratorPos) != '=' &&
           GetCharAt(iteratorPos) != '\n';
           --iteratorPos)
        ;
      if (GetCharAt(iteratorPos) != '<' && GetCharAt(iteratorPos) != '=')
        ++iteratorPos;
      SetSelection(iteratorPos, iteratorPos);
      return;
    } else
...
wxString XmlCtrl::getLastElementName(int pos) {
  ...
  if (GetCharAt(startPos) != '<' || GetCharAt(startPos + 1) == '/' ||
      GetCharAt(startPos + 1) == '?' || GetCharAt(startPos + 1) == '!')
    return _T ( "" );

  ++startPos;
  for (iteratorPos = startPos;
       iteratorPos < pos && GetCharAt(iteratorPos) != ' ' &&
       GetCharAt(iteratorPos) != '\n' && GetCharAt(iteratorPos) != '\t' &&
       GetCharAt(iteratorPos) != '/' && GetCharAt(iteratorPos) != '>';
       ++iteratorPos)
    ;
...
wxString XmlCtrl::getLastAttributeName(int pos) {
  ...
  if (GetCharAt(startPos) != ' ' || startPos >= pos - 1)
    return _T ( "" );

  return GetTextRange(startPos + 1, pos);
}

int XmlCtrl::findNextEndTag(int pos, unsigned depth /*= 1*/,
                            int character /*= '>'*/,
...
int XmlCtrl::findPreviousStartTag(int pos, unsigned depth /*= 1*/,
                                  int character /*= '<'*/,
                                  int range /*= USHRT_MAX * 4*/) {
  wxASSERT(character == '<' || character == '>');
...
void XmlCtrl::applyProperties(const XmlCtrlProperties &propertiesParameter,
                              bool zoomOnly) {
  ...
  SetIndentationGuides((type != FILE_TYPE_BINARY) ? properties.indentLines
                                                  : false);
  SetWrapMode((type != FILE_TYPE_BINARY) ? properties.wrap : false);
  SetViewWhiteSpace((properties.whitespaceVisible) ? wxSTC_WS_VISIBLEAFTERINDENT
                                                   : wxSTC_WS_INVISIBLE);
  ...
  setColorScheme((type != FILE_TYPE_BINARY) ? properties.colorScheme
                                            : COLOR_SCHEME_NONE);
  ...
  if (properties.fold && type != FILE_TYPE_BINARY) {
    ...
    MarkerDefine(wxSTC_MARKNUM_FOLDEREND, wxSTC_MARK_BOXPLUSCONNECTED, clrWnd,
                 clrText);
    MarkerDefine(wxSTC_MARKNUM_FOLDEROPENMID, wxSTC_MARK_BOXMINUSCONNECTED,
                 clrWnd, clrText);
    MarkerDefine(wxSTC_MARKNUM_FOLDERMIDTAIL, wxSTC_MARK_TCORNER, clrWnd,
                 clrText);
    ...
    MarkerDefine(wxSTC_MARKNUM_FOLDEROPEN, wxSTC_MARK_BOXMINUS, clrWnd,
                 clrText);
  } else {
    SetMarginWidth(2, 0);
  }
  ...
  wxString value =
      (properties.fold && type != FILE_TYPE_BINARY) ? _T ( "1" ) : _T ( "0" );
...
bool XmlCtrl::isCloseTag(int pos) {
  ...
  for (iteratorPos = pos; iteratorPos >= 0; --iteratorPos) {
    if (GetCharAt(iteratorPos) == '<' &&
        getLexerStyleAt(iteratorPos) == wxSTC_H_TAG)
...
void XmlCtrl::setColorScheme(int scheme) {
  ...
  case COLOR_SCHEME_DEFAULT:
    ...
    alternateBackground =
        LightColour(wxTheColourDatabase->Find(_T ( "CYAN" )), 95);
    SetCaretLineBackground(baseBackground);

    SetCaretForeground(*wxBLACK);
    SetSelBackground(
        true, LightColour(
                  wxTheColourDatabase->Find(_T ( "YELLOW" )),
                  20)); // wxTheColourDatabase->Find ( _T ( "LIGHT GREY" ) ) );
  ...
  case COLOR_SCHEME_REDUCED_GLARE:
    ...
    alternateBackground =
        LightColour(wxTheColourDatabase->Find(_T ( "NAVY" )), 10);
    SetCaretLineBackground(baseBackground);
...
int XmlCtrl::getType() { return type; }

void XmlCtrl::foldAll() { expandFoldsToLevel(1, false); }
  ...
  for (int n = 0; n < line_n; n++) {
    ...
    if (XMLCTRL_HASBIT(line_level, wxSTC_FOLDLEVELHEADERFLAG)) {
      ...
      if (((expand && (line_level <= level)) ||
           (!expand && (line_level >= level))) &&
          (GetFoldExpanded(n) != expand))
        ToggleFold(n);
    }
  }
...
wxString XmlCtrl::getOpenTag(const wxString &element) {
  ...
  if (!requiredAttributeSet.empty()) {
    for (it = requiredAttributeSet.begin(); it != requiredAttributeSet.end();
         ++it) {
      openTag += _T ( " " );
      openTag += *it;
      openTag += _T ( "=\"\"" );
...
void XmlCtrl::toggleLineBackground() {
  ...
  SetCaretLineBackground((lineBackgroundState == BACKGROUND_STATE_NORMAL)
                             ? baseBackground
                             : alternateBackground);
}

const std::set< wxString > &XmlCtrl::getEntitySet() { return entitySet; }
...
bool XmlCtrl::backgroundValidate() {
  ...
  return backgroundValidate(bufferUtf8.c_str(), basePath, bufferUtf8.size());
}

bool XmlCtrl::backgroundValidate(const char *buffer, const wxString &system,
                                 size_t bufferLen) {
  ...
  validationThread = new ValidationThread(GetEventHandler(), buffer, system);
...
std::string XmlCtrl::myGetTextRaw() { return (const char *)GetTextRaw(); }
...
bool XmlCtrl::getValidationRequired() { return validationRequired; }

void XmlCtrl::setValidationRequired(bool b) { validationRequired = b; }
...
bool XmlCtrl::getGrammarFound() { return grammarFound; }
...
wxString XmlCtrl::getCurrentXPath() {
  ...
  for (;;) {
    ...
    if (pos != wxString::npos && pos < name.length()) {
      ++pos;
      name = _T("*[local-name()='") + name.substr(pos, name.length() - pos) +
             _T("']");
    }
    xpath = _T("/") + name + xpath;
  }
