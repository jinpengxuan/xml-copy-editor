...
EVT_UPDATE_UI_RANGE(ID_STYLE_EDIT, ID_STYLE_CHANGE_ALL,
                    StyleDialog::OnUpdateTableRange)
END_EVENT_TABLE()
...
StyleDialog::StyleDialog(wxWindow *parent, wxIcon icon,
...
#endif
                         int typeParameter, bool readOnlyParameter,
                         wxPoint position, wxSize size)
    : wxDialog(parent, wxID_ANY,
               wxString((typeParameter == ID_TYPE_STYLE) ? _("Style")
                                                         : _("Spelling")),
               position, size,
               wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxMAXIMIZE_BOX),
      indexForContextMenu(-1), bufferUtf8(bufferParameterUtf8),
#if !defined(USE_ENCHANT) && defined(__WXMSW__)
      aspellDataPath(aspellDataPathParameter),
      aspellDictPath(aspellDictPathParameter),
#endif
      fileName(fileNameParameter), ruleSetDirectory(ruleSetDirectoryParameter),
      filterDirectory(filterDirectoryParameter),
      ruleSetPreset(ruleSetPresetParameter),
      filterPreset(filterPresetParameter), type(typeParameter),
      readOnly(readOnlyParameter) {
  SetIcon(icon);

  // top box
  ruleSetCombo = new wxComboBox(this, ID_STYLE_COMBO_RULESET, _T ( "" ),
  ...
  filterCombo = new wxComboBox(this, ID_STYLE_COMBO_FILTER, _T ( "" ),
                               wxDefaultPosition, wxSize(200, -1));
  // if (type != ID_TYPE_STYLE) // from v. 1.1.0.7: never show
  filterCombo->Show(false);

  wxButton *createReportButton = new wxButton(
      this, ID_STYLE_REPORT, _("&Check"), wxDefaultPosition, buttonSize, 0);
  ...
  // middle box
  wxListCtrl *myTable = new wxListCtrl(this, ID_STYLE_TABLE, wxPoint(0, 0),
  ...
  wxButton *editItemsButton =
      new wxButton(this, ID_STYLE_EDIT, _("&Apply changes"), wxDefaultPosition,
                   wxSize(-1, buttonSize.GetHeight()), 0);
  wxButton *webReportButton =
      new wxButton(this, ID_STYLE_WEB_REPORT, _("&Printable report"),
                   wxDefaultPosition, wxSize(-1, buttonSize.GetHeight()), 0);
  wxButton *webSummaryButton =
      new wxButton(this, ID_STYLE_WEB_SUMMARY, _("Pr&intable summary"),
                   wxDefaultPosition, wxSize(-1, buttonSize.GetHeight()), 0);
  wxButton *selectAllButton =
      new wxButton(this, ID_STYLE_CHANGE_ALL, _("C&hange all"),
                   wxDefaultPosition, wxSize(-1, buttonSize.GetHeight()), 0);
  wxButton *deselectAllButton =
      new wxButton(this, ID_STYLE_IGNORE_ALL, _("I&gnore all"),
                   wxDefaultPosition, wxSize(-1, buttonSize.GetHeight()), 0);
  wxButton *cancelButton =
      new wxButton(this, wxID_CANCEL, _("Ca&ncel"), wxDefaultPosition,
                   wxSize(-1, buttonSize.GetHeight()), 0);
  ...
  if (type == ID_TYPE_SPELL) {
    ...
    aspell_config_replace(config, "data-dir",
                          aspellDataPath.mb_str()); // ASPELL_DATA_PATH );
    aspell_config_replace(config, "dict-dir",
                          aspellDictPath.mb_str()); // ASPELL_DICT_PATH );
#endif
  ...
  if (wxDirExists(ruleSetDirectory)) {
    ...
    ruleMask =
        ruleSetDirectory + wxFileName::GetPathSeparator() + _T ( "*.xml" );
  ...
  if (wxDirExists(filterDirectory)) {
    ...
    filterMask =
        filterDirectory + wxFileName::GetPathSeparator() + _T ( "*.xml" );
...
void StyleDialog::OnItemRightClick(wxListEvent &event) {
  ...
  wxMenu contextMenu;
  contextMenu.Append(ID_MENU_IGNORE_ONCE, _("Ignore once"));
  contextMenu.Append(ID_MENU_IGNORE_ALL, _("Ignore all"));
  contextMenu.AppendSeparator();
  contextMenu.Append(ID_MENU_CHANGE_ONCE, _("Change once"));
  contextMenu.Append(ID_MENU_CHANGE_ALL, _("Change all"));
  contextMenu.AppendSeparator();
  contextMenu.Append(ID_MENU_NEW_SUGGESTION, _("New suggestion..."));

  wxString menuString;
  menuString.Printf(_T ( "Change '%s' to '%s' throughout" ), match.c_str(),
                    suggestion.c_str());

  contextMenu.Append(ID_MENU_APPLY_SUGGESTION_ALL, menuString);

  bool ignore = (getTextByColumn(table, index, 6) == _("Ignore"));
  contextMenu.Enable((ignore) ? ID_MENU_IGNORE_ONCE : ID_MENU_CHANGE_ONCE,
                     false);
...
void StyleDialog::OnReport(wxCommandEvent &event) {
  ...
  std::auto_ptr< HouseStyle > hs(new HouseStyle(
      (type == ID_TYPE_SPELL) ? HS_TYPE_SPELL : HS_TYPE_STYLE, bufferUtf8,
      ruleSetDirectory, ruleSet, filterDirectory, filter, separator,
#ifdef __WXMSW__
...
std::string StyleDialog::getEditedString() { return bufferUtf8; }
...
void StyleDialog::OnStyleWebSummary(wxCommandEvent &event) {
  ...
  for (vectorIterator = v.begin(); vectorIterator != v.end();
       ++vectorIterator) {
  ...
  for (mapIterator = matchMap.begin(); mapIterator != matchMap.end();
       ++mapIterator) {
    ofs << "<tr><td>";
...
bool StyleDialog::elementAndOffsetCompareFunction(ContextMatch m1,
                                                  ContextMatch m2) {
...
void StyleDialog::OnMenuNewSuggestion(wxCommandEvent &event) {
  ...
  wxTextEntryDialog *dlg = new wxTextEntryDialog(
      this, _("Enter new suggestion:"), _("New Suggestion"), suggestion);
  if (!dlg)
    return;
...
wxPoint StyleDialog::getPosition() { return framePosition; }

wxSize StyleDialog::getSize() { return frameSize; }
