...
XmlTextInfo::XmlTextInfo(const wxString &path, const char *encoding /*= NULL*/
                         )
    : WrapExpat(encoding) {
...
void XMLCALL XmlTextInfo::startdoctypehandler(void *data,
...
void XMLCALL XmlTextInfo::processinghandler(void *data, const XML_Char *target,
...
void XMLCALL XmlTextInfo::start(void *data, const XML_Char *el,
                                const XML_Char **attr) {
  XmlTextInfo *pThis = (XmlTextInfo *)data;
  ...
  for (; *attr; attr += 2) {
    ...
    if (strstr(*attr, ":schemaLocation")) { // TODO: Support multiple schemas
...
void XMLCALL XmlTextInfo::defaulthandler(void *data, const XML_Char *s,
                                         int len) {
  XmlTextInfo *pThis = (XmlTextInfo *)data;
  const char *p = s;
