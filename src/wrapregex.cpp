...
WrapRegex::WrapRegex(const string &pattern, bool matchCase,
                     const string &replaceParameter,
                     const int arrayLengthParameter)
    : replace(replaceParameter), arrayLength(arrayLengthParameter),
      returnValue(0) {
  ...
  if ((patternStructure = pcre_compile(pattern.c_str(), optionsFlag,
                                       &errorPointer, &errorOffset, NULL)) ==
      NULL) {
...
int WrapRegex::matchPatternGlobal(string &buffer,
                                  vector< ContextMatch > &matchVector,
                                  unsigned elementCount, int context) {
  if (disabled)
    return 0;

  return matchPatternGlobal_(buffer.c_str(), buffer.size(), matchVector,
                             elementCount, context);
}

string WrapRegex::replaceGlobal(const string &buffer, int *matchCount) {
  *matchCount = 0;
  ...
  while ((returnValue = pcre_exec(patternStructure, patternExtraStructure, s,
                                  strlen(s), 0, 0, matchArray, arrayLength)) >=
         0) {
    ++(*matchCount);
...
int WrapRegex::matchPatternGlobal_(const char *buffer, size_t buflen,
                                   vector< ContextMatch > &matchVector,
                                   unsigned elementCount, int context) {
  if (disabled)
    return 0;
  ...
  while ((returnValue =
              pcre_exec(patternStructure, patternExtraStructure, s, buflen,
                        offset, 0, matchArray, arrayLength)) >= 0) {
    ++matchcount;

    if (context) {
      match = ContextHandler::getContext(
          s + matchArray[0], matchArray[1] - matchArray[0], origin, context);
    } else {
...
string WrapRegex::getInterpolatedString_(const char *buffer,
                                         const char *source) {
  if (disabled)
    return "";
