...
public:
  XmlPromptGenerator(wxEvtHandler *handler, const char *buffer,
                     size_t bufferSize,
  ...
  const std::map< wxString, std::set< wxString > > &getRequiredAttributeMap() {
    return d->requiredAttributeMap;
  }
  const std::map< wxString, std::set< wxString > > &getElementMap() {
    return d->elementMap;
  }
  const std::set< wxString > &getEntitySet() { return d->entitySet; }
  bool getGrammarFound() { return d->grammarFound; }
  const std::map< wxString, wxString > &getElementStructureMap() {
...
private:
  ...
  static void XMLCALL starthandler(void *data, const XML_Char *el,
                                   const XML_Char **attr);
  static void XMLCALL endhandler(void *data, const XML_Char *el);
  static void XMLCALL doctypedeclstarthandler(void *userData,
  ...
  static void XMLCALL elementdeclhandler(void *userData, const XML_Char *name,
                                         XML_Content *model);
  void getContent(const XML_Content &content, wxString &contentModel,
  ...
  int parseGrammar(PromptGeneratorData *d, const wxString &publicId,
                   const wxString &systemId, const wxString &baseFileName,
                   xercesc::Grammar::GrammarType type);
  static int XMLCALL externalentityrefhandler(XML_Parser p,
  ...
  void handleSchema(PromptGeneratorData *d, const XML_Char *el,
                    const XML_Char **attr);
  void buildSubstitutionMap(SubstitutionMap &substitutions,
                            const xercesc::SchemaGrammar &grammar);
  void buildElementPrompt(PromptGeneratorData *d,
                          const xercesc::XMLElementDecl *element,
                          SubstitutionMap &substitutions);
  void getContent(std::set< wxString > &list,
