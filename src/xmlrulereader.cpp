...
RuleData::RuleData(
    ...
    boost::shared_ptr< vector< boost::shared_ptr< Rule > > >
        ruleVectorParameter)
...
XmlRuleReader::XmlRuleReader(
    ...
    boost::shared_ptr< vector< boost::shared_ptr< Rule > > >
        ruleVectorParameter)
    : ud(new RuleData(dictionaryParameter, passiveDictionaryParameter,
                      ruleVectorParameter)) {
...
int XmlRuleReader::getRuleCount() { return ud->ruleCount; }
...
void XMLCALL XmlRuleReader::start(void *data, const XML_Char *el,
                                  const XML_Char **attr) {
  RuleData *ud;
  ud = (RuleData *)data;
...
void XMLCALL XmlRuleReader::end(void *data, const XML_Char *el) {
  ...
  // handle end of rule
    ...
    try {
      boost::shared_ptr< Rule > rule(
          new Rule(ud->find, ud->matchcase, ud->replace));
      ...
      ud->incorrectPatternReport =
          "Cannot compile: " + ud->find + "\r\nError: " + e.what();
...
void XMLCALL XmlRuleReader::characterdata(void *data, const XML_Char *s,
                                          int len) {
  RuleData *ud;
  ud = (RuleData *)data;
