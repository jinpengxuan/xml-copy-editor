...
WrapDaisy::WrapDaisy(MyFrame *frameParameter, const wxString &daisyDirParameter,
  ...
  daisyCwd =
      daisyDir + _T("pipeline-20090410") + wxFileName::GetPathSeparator();
  ...
  baseCmd =
      _T("java -classpath ") + classPath + _T( " " ) + commandLineUI + _T(" ");
}

WrapDaisy::~WrapDaisy() { wxFileName::SetCwd(memoryCwd); }

bool WrapDaisy::run(wxString &fileIn, wxString &stylesheet, wxString &folder,
                    bool quiet, bool suppressOptional, bool epub, bool rtf,
                    bool doc, bool fullDaisy, bool mp3Album) {
  fileIn.Replace(_T("\\"), _T("/"));
  stylesheet.Replace(_T("\\"), _T("/"));

  std::auto_ptr< wxProgressDialog > pd(
      new wxProgressDialog(_("Export in progress"), _("Initializing..."), 100,
                           NULL, wxPD_SMOOTH | wxPD_CAN_ABORT));

  // prepare dtbook location
  wxString dtbDir, dtbFilePath;

  dtbDir = folder + wxFileName::GetPathSeparator() + _T("dtbook");
  dtbFilePath = dtbDir + wxFileName::GetPathSeparator() + _T("dtbook.xml");
...
#else
  if (wxMkDir((const char *)imagesDir.mb_str(wxConvUTF8), 0) &&
      !imagesDirExists)
#endif
  ...
  wxString canonicalFile =
      htmlDir + wxFileName::GetPathSeparator() + _T("index.html");
  std::string stdCanonicalFile = (const char *)canonicalFile.mb_str(wxConvUTF8);
  std::ofstream canonicalStream(
      stdCanonicalFile.c_str()); // canonicalFile.name().c_str() );
  if (!canonicalStream) {
  ...
  wxString cmd = baseCmd + xhtml2dtbookScript + _T(" --inputFile=\"") +
                 canonicalFile + _T("\" --outputFile=\"") + dtbFilePath +
                 _T("\"");
  ...
  if (epub) {
    ...
    cmd = baseCmd + _T("\"") + epubScript + _T("\" --input=\"") +
          canonicalFile + // canonicalFile.wideName() +
          _T("\" --output=\"") + folder + wxFileName::GetPathSeparator() +
          _T("ebook.epub\"");
  ...
  if (rtf || doc) {
    ...
    cmd = baseCmd + _T("\"") + rtfScript + _T("\" --input=\"") +
  ...
  cmd = baseCmd + _T("\"") + narratorScript + _T("\" --input=\"") +
        dtbFilePath + _T("\"") + // dtbFile.wideName() +
        _T(" --outputPath=") + _T("\"") + folder + _T("\"");
  ...
  */
  ...
  folderWith202SmilFile = folder + wxFileName::GetPathSeparator() +
  ...
  cmd =
      baseCmd + _T("\"") + taggerScript + _T("\" --audioTaggerInputFile=\"") +
      fileWith202SmilAttribs +
      _T("\" --audioTaggerOutputPath=") + // see filesetAudioTagger transformer
      _T("\"") + albumDir + _T("\"");
  ...
  */
  ...
  /*
      ...
      fileWithSmilAttribs = folderWithSmilFile + fn.GetFullName();
          ...
          {
                  error.Printf ( _ ( "Cannot open %s" ),
  fileWithSmilAttribs.c_str() ); return false;
          }
      ...
      wxString wideAlbumTitle = wxString ( albumTitle.c_str(), wxConvUTF8,
  albumTitle.size() ); albumDir += wideAlbumTitle; #ifdef __WXMSW__
  ...
  #else
      if ( !wxMkDir ( (const char *) albumDir.mb_str( wxConvUTF8 ), 0 ) &&
  !albumDirExists ) #endif
      {
      ...
      for ( size_t i = 0; i < vectorSize; i++ )
      {
          ...
          wideSourceFile = wxString ( sourceFile.c_str(), wxConvUTF8,
  sourceFile.size() ); wideDestFile = wxString ( destFile.c_str(), wxConvUTF8,
  destFile.size() );
...
const wxString &WrapDaisy::getLastError() { return error; }
