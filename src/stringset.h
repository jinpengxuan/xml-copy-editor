...
template < class T > class StringSetNode : private boost::noncopyable {
public:
...
public:
  StringSet(int hashSizePower = 19);
  ~StringSet();
...
template < class T > StringSet< T >::StringSet(int hashSizePower) {
  if (hashSizePower < 1)
...
template < class T > StringSet< T >::~StringSet() {
  if (!empty())
    clear();
  delete[] table;
}

template < class T > StringSet< T >::StringSet(const StringSet< T > &d) {
  hashSize = d.hashSize;
  hashMask = hashSize - 1;
  nodeCount = 0;
  allocateHashTable(hashSize);
...
template < class T >
StringSetNode< T > *StringSet< T >::insert(T *s, size_t len,
                                           uint32_t hashValue) {
  if (hashValue == UINT_MAX)
    hashValue = hash((const char *)s, len * sizeof(T));
  StringSetNode< T > *np;
...
template < class T > void StringSet< T >::clear() {
  if (!nodeCount)
    return;
...
template < class T > int StringSet< T >::count() { return nodeCount; }

template < class T > bool StringSet< T >::empty() {
  return (nodeCount) ? false : true;
}

template < class T > void StringSet< T >::allocateHashTable(uint32_t hashSize) {
