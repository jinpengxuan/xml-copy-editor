...
void XMLCALL Mp3Album::start(void *data, const XML_Char *el,
                             const XML_Char **attr) {
  Mp3AlbumData *ad;
  ...
  for (; *attr; attr += 2) {
    ...
    if (!strcmp(*attr, "smilref") && !strncmp(*(attr + 1), "speechgen", 9)) {
...
void XMLCALL Mp3Album::end(void *data, const XML_Char *el) {
  ...
  if (ad->armed) {
    ...
    ad->fileNameVector.push_back(
        std::make_pair(source, destinationStream.str()));
  }
...
void XMLCALL Mp3Album::characterdata(void *data, const XML_Char *s, int len) {
  Mp3AlbumData *ad;
  ad = (Mp3AlbumData *)data;
...
void Mp3Album::getFileNameVector(
    std::vector< std::pair< std::string, std::string > > &v) {
  v = d->fileNameVector;
}

std::string Mp3Album::getAlbumTitle() { return d->albumTitle; }
