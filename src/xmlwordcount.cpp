...
XmlWordCount::XmlWordCount(const char *encoding)
    : WrapExpat(encoding, false), wcd(new WordCountData()) {
  wcd->wordCount = 0;
...
int XmlWordCount::getWordCount() { return wcd->wordCount; }

void XMLCALL XmlWordCount::characterdata(void *data, const XML_Char *s,
                                         int len) {
  WordCountData *wcd;
