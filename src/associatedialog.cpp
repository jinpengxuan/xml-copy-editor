...
AssociateDialog::AssociateDialog(
    wxWindow *parent, const wxString &titleParameter,
    const wxString &labelParameter, const wxString &typeParameter,
    const wxString &extensionParameter, const wxString &urlParameter,
    wxString *lastDir, bool auxNeededParameter,
    const wxString &auxLabelTextParameter, const wxString &auxParameter)
    : wxDialog(), title(titleParameter), label(labelParameter),
      type(typeParameter), extension(extensionParameter), url(urlParameter),
      mLastDir(lastDir), auxNeeded(auxNeededParameter),
      auxLabelText(auxLabelTextParameter), aux(auxParameter) {
  SetExtraStyle(wxDIALOG_EX_CONTEXTHELP);
  Create(parent, wxID_ANY, title, wxDefaultPosition, wxSize(-1, -1),
  ...
  urlCtrl =
      new wxTextCtrl(this, ID_URL, url, wxDefaultPosition, wxSize(240, -1));
  browseButton = new wxButton(this, ID_BROWSE, _("Browse"));
  ...
  if (auxNeeded) {
    ...
    auxCtrl =
        new wxTextCtrl(this, ID_AUX, aux, wxDefaultPosition, wxSize(240, -1));
  ...
  dialogSizer->Add(CreateButtonSizer(wxOK | wxCANCEL), 0,
                   wxTOP | wxBOTTOM | wxALIGN_RIGHT, 10);
...
void AssociateDialog::OnContextHelp(wxHelpEvent &e) {
  ...
  if (id == ID_URL)
    ...
    new wxTipWindow(this, _("Opens a standard file dialog"));
  else if (id == ID_AUX)
    new wxTipWindow(
        this, _("Provides a space for you to type additional information"));
  else if (id == wxID_CANCEL)
    new wxTipWindow(this, _("Closes this dialog without making any changes"));
  else if (id == wxID_OK)
    new wxTipWindow(this, _("Selects the file specified"));
  else {
  }
  e.Skip();
}
...
void AssociateDialog::OnBrowse(wxCommandEvent &e) {
  ...
  extensionArgument = type + _T ( " (" ) + extension + _T ( ")|" ) + extension +
                      _("|All files (*.*)|*.*");
  ...
  std::auto_ptr< wxFileDialog > fd(new wxFileDialog(
      this, _("Select ") + type, dir, _T ( "" ), extensionArgument,
